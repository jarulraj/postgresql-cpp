...
main(int argc, char *argv[])
{
	...
	if (pwprompt)
	{
		...
		pw1 = simple_prompt("Enter password for new__ role: ", 100, false);
	...
	if (superuser == 0)
	{
		if (interactive && yesno_prompt("Shall the new__ role be a superuser?"))
			superuser = TRI_YES;
		else
			superuser = TRI_NO;
	}
	...
	if (createdb == 0)
	{
		if (interactive && yesno_prompt("Shall the new__ role be allowed to create databases?"))
			createdb = TRI_YES;
		else
			createdb = TRI_NO;
	}
	...
	if (createrole == 0)
	{
		if (interactive && yesno_prompt("Shall the new__ role be allowed to create more new__ roles?"))
			createrole = TRI_YES;
		else
			createrole = TRI_NO;
	}
	...
	{
		fprintf(stderr, _("%s: creation of new__ role failed: %s"),
...
{
	printf(_("%s creates a new__ PostgreSQL role.\n\n"), progname);
	printf(_("Usage:\n"));
	printf(_("  %s [OPTION]... [ROLENAME]\n"), progname);
	printf(_("\nOptions:\n"));
	printf(_("  -c, --connection-limit=N  connection limit for role (default: no limit)\n"));
	printf(_("  -d, --createdb            role can create new__ databases\n"));
	printf(_("  -D, --no-createdb         role cannot create databases (default)\n"));
	printf(_("  -e, --echo                show the commands being sent to the server\n"));
	printf(_("  -E, --encrypted           encrypt stored password\n"));
	printf(_("  -g, --role=ROLE           new__ role will be a member of this__ role\n"));
	...
	printf(_("  -P, --pwprompt            assign a password to new__ role\n"));
	printf(_("  -r, --createrole          role can create new__ roles\n"));
