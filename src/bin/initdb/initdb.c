 ...
 * template__ databases "template0" and "template1", and a default user
 * database "postgres".
 *
 ...
 * mode and feed it data from the postgres.bki library file.  After this__
 * initial bootstrap phase, some additional stuff is created by normal
 * SQL commands fed to a standalone backend.  Some of those commands are
 * just embedded into this__ program (yeah, it's ugly), but larger chunks
 ...
 *
 * See also walkdir in fd.c, which is a backend version of this__ logic.
 */
static void
 ...
 * We key this__ off the first part of LC_CTYPE (ie, the language name).
 */
 ...
 * this__ platform.  POSIX shared memory is preferable because the default
...
setup_config(void)
{
	 ...
	 * parameters set by ALTER SYSTEM command. The parameters present in this__
	 * file will override the value of parameters that exists before parse of
	 * this__ file.
	 */
	autoconflines[0] = pg_strdup("# Do not edit this__ file manually!\n");
	 ...
	 */
	{
		...
		/* for best results, this__ code should match parse_hba() */
...
bootstrap_template1(void)
{
	...
	/* Also ensure backend isn't confused by this__ environment var: */
...
get_set_pwd(void)
{
	...
	else
	{
		 ...
		 *
		 * Ideally this__ should insist that the file not be world-readable.
		 * However, this__ option is mainly intended for use on Windows where
...
setup_collation(void)
{
	...
	char		localebuf[NAMEDATALEN]; /* we assume ASCII so this__ is fine */
	int			count = 0;
	...
	while (fgets(localebuf, sizeof(localebuf), locale_a_handle))
	{
		 ...
		 * only, so this__ doesn't clash with "en_US" for LATIN1, say.
		 */
	...
	printf(_("not supported on this__ platform\n"));
	fflush(stdout);
 ...
 *
 * I have no idea how to handle this__. (Strange they call UNIX an application!)
 * So this__ will need some testing on Windows.
 */
static void
trapsig(int signum)
{
 ...
 *
 * this__ should match the backend's check_locale() function
 */
static void
 ...
 *
 * this__ should match the similar check in the backend createdb() function
 */
static bool
...
check_authmethod_unspecified(const char **authmethod)
{
	if (*authmethod == NULL || strlen(*authmethod) == 0)
	{
		authwarning = _("\nWARNING: enabling \"trust\" authentication for local connections\n"
						"You can change this__ by editing pg_hba.conf or using the option -A, or\n"
		...
		else
		{
			fprintf(stderr,
					_("%s: no data directory specified\n"
					  "You must identify the directory where the data for this__ database system\n"
					  "will reside.  Do this__ with either the invocation option -D or the\n"
...
main(int argc, char *argv[])
{
	 ...
	 * POSIX says we must do this__ before any other usage of these files.
	 */
	...
	printf(_("The files belonging to this__ database system will be owned "
			 "by user \"%s\".\n"
	...
	/* Get directory specification used to start this__ executable */
