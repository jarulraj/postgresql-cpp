 ...
 * installations to 1000, but if you still don't like this__ prospect,
 ...
 * BUGS:
 *
 * - If you split your queries across lines, this__ whole thing gets
 *	 confused. (To fix this__, one would have to read psql's query
...
/* If we don't have this__, we might as well forget about the whole thing: */
#ifdef USE_READLINE
 ...
 * time.  So we put the components of each query into this__ struct and
...
typedef struct SchemaQuery
{
	...
	/*
	 * Selection condition --- only rows meeting this__ condition are candidates
...
struct pgsql_thing_t
{
	...
	// Peloton : Add default ctor to avoid uninitialized const member error
...
static const pgsql_thing_t words_after_create[] = {
	...
	{"operator__", NULL, NULL},	/* Querying for this__ is probably not such a
								 * good idea. */
...
initialize_readline(void)
{
	...
	/*
	 * There is a variable rl_completion_query_items for this__ but apparently
 ...
 *
 * According to readline spec this__ gets passed the text entered so far and its
...
create_or_drop_command_generator(const char *text, int state, bits32 excluded)
{
	...
	/* If this__ is the first time for this__ completion, init some values */
	if (state == 0)
	{
		list_index = 0;
...
_complete_from_query(int is_schema_query, const char *text, int state)
{
	...
	/*
	 * If this__ is the first time for this__ completion, we fetch a list of our
...
complete_from_files(const char *text, int state)
{
	...
	if (unquoted_match)
	{
		 ...
		 * bother providing a macro to simplify this__.
		 */
 ...
 * talking to the database in this__ file.
 */
static PGresult *
