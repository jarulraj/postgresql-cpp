...
MainLoop(FILE *source)
{
	...
	volatile PQExpBuffer previous_buf;	/* if there isn't anything in the new__
	...
	while (successResult == EXIT_SUCCESS)
	{
		 ...
		 * new__ line of input (note it must be freed before looping around!)
		 */
		...
		while (success || !die_on_error)
		{
			...
			else if (scan_result == PSCAN_BACKSLASH)
			{
				...
				{
					/* rescan query_buf as new__ input */
					psql_scan_finish(scan_state);
					free(line);
