...
static void dumpComment(Archive *fout, const char *target,
			const char *namespace__, const char *owner,
...
static void dumpUserMappings(Archive *fout,
				 const char *servername, const char *namespace__,
 ...
 *		Mark a namespace__ as to be dumped or not
 */
static void
selectDumpableNamespace(NamespaceInfo *nsinfo, DumpOptions *dopt)
{
	...
	/*
	 * In any case, a namespace__ can be excluded by an exclusion switch
	 */
	if (nsinfo->dobj.dump &&
...
selectDumpableTable(TableInfo *tbinfo, DumpOptions *dopt)
{
	 ...
	 * according to the parent namespace__'s dump flag.
	 */
	if (table_include_oids.head != NULL)
		tbinfo->dobj.dump = simple_oid_list_member(&table_include_oids,
												   tbinfo->dobj.catId.oid);
	else
		tbinfo->dobj.dump = tbinfo->dobj.namespace__->dobj.dump;
...
selectDumpableType(TypeInfo *tyinfo, DumpOptions *dopt)
{
	...
	if (!tyinfo->dobj.namespace__->dobj.dump)
		tyinfo->dobj.dump = false;
	else
...
selectDumpableDefaultACL(DefaultACLInfo *dinfo, DumpOptions *dopt)
{
	...
	if (dinfo->dobj.namespace__)
		dinfo->dobj.dump = dinfo->dobj.namespace__->dobj.dump;
	else
 ...
 *
 * Casts do not belong to any particular namespace__ (since they haven't got
 ...
 *
 * Procedural languages do not belong to any particular namespace__.  To
...
selectDumpableObject(DumpableObject *dobj, DumpOptions *dopt)
{
	...
	/*
	 * Default policy is to dump if parent namespace__ is dumpable, or for
	 * non-namespace__-associated items, dump if we're dumping "everything".
	 */
	if (dobj->namespace__)
		dobj->dump = dobj->namespace__->dobj.dump;
	else
...
dumpTableData_copy(Archive *fout, void *dcontext)
{
	...
	if (g_verbose)
		write_msg(NULL, "dumping contents of table \"%s.%s\"\n",
				  tbinfo->dobj.namespace__->dobj.name, classname);
	 ...
	 */
	selectSourceSchema(fout, tbinfo->dobj.namespace__->dobj.name);
	...
	if (oids && hasoids)
	{
		appendPQExpBuffer(q, "COPY %s %s WITH OIDS TO stdout;",
						  fmtQualifiedId(fout->remoteVersion,
										 tbinfo->dobj.namespace__->dobj.name,
										 classname),
						  column_list);
	}
...
dumpTableData(Archive *fout, TableDataInfo *tdinfo)
{
	...
	ArchiveEntry(fout, tdinfo->dobj.catId, tdinfo->dobj.dumpId,
				 tbinfo->dobj.name, tbinfo->dobj.namespace__->dobj.name,
				 NULL, tbinfo->rolname,
...
refreshMatViewData(Archive *fout, TableDataInfo *tdinfo)
{
	...
	ArchiveEntry(fout,
				 ...
				 tbinfo->dobj.namespace__->dobj.name,		/* namespace__ */
				 NULL,			/* Tablespace */
...
makeTableDataInfo(DumpOptions *dopt, TableInfo *tbinfo, bool oids)
{
	...
	tdinfo->dobj.namespace__ = tbinfo->dobj.namespace__;
...
getPolicies(Archive *fout, TableInfo tblinfo[], int numTables)
{
	...
	for (i = 0; i < numTables; i++)
	{
		...
		if (tbinfo->rowsec)
		{
			...
			polinfo->dobj.namespace__ = tbinfo->dobj.namespace__;
		...
		for (j = 0; j < ntups; j++)
		{
			...
			polinfo[j].dobj.namespace__ = tbinfo->dobj.namespace__;
...
dumpPolicy(Archive *fout, PolicyInfo *polinfo)
{
	...
	if (polinfo->polname == NULL)
	{
		...
		ArchiveEntry(fout, polinfo->dobj.catId, polinfo->dobj.dumpId,
					 polinfo->dobj.name,
					 polinfo->dobj.namespace__->dobj.name,
					 NULL,
...
getNamespaces(Archive *fout, int *numNamespaces)
{
	 ...
	 * read in can be linked to a containing namespace__.
	 */
	...
	for (i = 0; i < ntups; i++)
	{
		...
		/* Decide whether to dump this namespace__ */
...
/*
 * findNamespace:
 *		given a namespace__ OID and an object OID, look up the info read by
 *		getNamespaces
 *
...
getExtensions(Archive *fout, int *numExtensions)
{
	...
	for (i = 0; i < ntups; i++)
	{
		...
		extinfo[i].namespace__ = pg_strdup(PQgetvalue(res, i, i_nspname));
...
getTypes(Archive *fout, int *numTypes)
{
	...
	for (i = 0; i < ntups; i++)
	{
									...
									tyinfo[i].typtype == TYPTYPE_RANGE))
		{
			...
			stinfo->dobj.namespace__ = tyinfo[i].dobj.namespace__;
...
getAggregates(Archive *fout, int *numAggs)
{
	...
	*numAggs = ntups;
	...
	for (i = 0; i < ntups; i++)
	{
		...
		agginfo[i].aggfn.dobj.namespace__ =
			findNamespace(fout,
	...
	for (i = 0; i < ntups; i++)
	{
		...
		if (tblinfo[i].dobj.dump && tblinfo[i].relkind == RELKIND_RELATION)
		{
			...
			appendPQExpBuffer(query,
							  ...
							  fmtQualifiedId(fout->remoteVersion,
										tblinfo[i].dobj.namespace__->dobj.name,
											 tblinfo[i].dobj.name));
			ExecuteSqlStatement(fout, query->data);
		}
...
getIndexes(Archive *fout, TableInfo tblinfo[], int numTables)
{
	...
	for (i = 0; i < numTables; i++)
	{
		...
		for (j = 0; j < ntups; j++)
		{
			...
			indxinfo[j].dobj.namespace__ = tbinfo->dobj.namespace__;
			...
			if (contype == 'p' || contype == 'u' || contype == 'x')
			{
				...
				constrinfo[j].dobj.namespace__ = tbinfo->dobj.namespace__;
...
getDomainConstraints(Archive *fout, TypeInfo *tyinfo)
{
	 ...
	 * qualified
	 */
	selectSourceSchema(fout, tyinfo->dobj.namespace__->dobj.name);
	...
	for (i = 0; i < ntups; i++)
	{
		...
		constrinfo[i].dobj.namespace__ = tyinfo->dobj.namespace__;
...
getRules(Archive *fout, int *numRules)
{
	...
	*numRules = ntups;
	...
	for (i = 0; i < ntups; i++)
	{
		...
		ruleinfo[i].dobj.namespace__ = ruleinfo[i].ruletable->dobj.namespace__;
...
getTriggers(Archive *fout, TableInfo tblinfo[], int numTables)
{
	...
	for (i = 0; i < numTables; i++)
	{
		...
		for (j = 0; j < ntups; j++)
		{
			...
			tginfo[j].dobj.namespace__ = tbinfo->dobj.namespace__;
	...
	for (i = 0; i < numTables; i++)
	{
		...
		if (hasdefaults)
		{
			...
			for (j = 0; j < numDefaults; j++)
			{
				...
				attrdefs[j].dobj.namespace__ = tbinfo->dobj.namespace__;
		...
		if (tbinfo->ncheck > 0)
		{
			...
			for (j = 0; j < numConstrs; j++)
			{
				...
				constrs[j].dobj.namespace__ = tbinfo->dobj.namespace__;
...
getForeignDataWrappers(Archive *fout, int *numForeignDataWrappers)
{
	...
	*numForeignDataWrappers = ntups;
	...
	for (i = 0; i < ntups; i++)
	{
		...
		fdwinfo[i].dobj.namespace__ = NULL;
...
getForeignServers(Archive *fout, int *numForeignServers)
{
	...
	*numForeignServers = ntups;
	...
	for (i = 0; i < ntups; i++)
	{
		...
		srvinfo[i].dobj.namespace__ = NULL;
...
getDefaultACLs(Archive *fout, int *numDefaultACLs)
{
	...
	*numDefaultACLs = ntups;
	...
	for (i = 0; i < ntups; i++)
	{
		...
		if (nspid != InvalidOid)
			daclinfo[i].dobj.namespace__ = findNamespace(fout, nspid,
												 daclinfo[i].dobj.catId.oid);
		else
			daclinfo[i].dobj.namespace__ = NULL;
 ...
 * the namespace__ and owner of the object (for labeling the ArchiveEntry),
...
/*
 * dumpNamespace
 *	  writes out to fout the queries to recreate a user-defined namespace__
 */
static void
dumpNamespace(Archive *fout, NamespaceInfo *nspinfo)
{
	...
	/* don't dump dummy namespace__ from pre-7.3 source */
...
dumpExtension(Archive *fout, ExtensionInfo *extinfo)
{
	...
	if (!dopt->binary_upgrade)
	{
		...
		appendPQExpBuffer(q, "CREATE EXTENSION IF NOT EXISTS %s WITH SCHEMA %s;\n",
						  qextname, fmtId(extinfo->namespace__));
	}
	else
	{
		int			i;
		...
		appendPQExpBufferStr(q, ", ");
		appendStringLiteralAH(q, extinfo->namespace__, fout);
		appendPQExpBufferStr(q, ", ");
...
dumpEnumType(Archive *fout, TypeInfo *tyinfo)
{
	...
	appendPQExpBuffer(delq, "DROP TYPE %s.",
					  fmtId(tyinfo->dobj.namespace__->dobj.name));
	...
	ArchiveEntry(fout, tyinfo->dobj.catId, tyinfo->dobj.dumpId,
				 tyinfo->dobj.name,
				 tyinfo->dobj.namespace__->dobj.name,
				 NULL,
	...
	dumpComment(fout, labelq->data,
				tyinfo->dobj.namespace__->dobj.name, tyinfo->rolname,
...
dumpRangeType(Archive *fout, TypeInfo *tyinfo)
{
	...
	if (OidIsValid(collationOid))
	{
		...
		if (coll)
		{
			/* always schema-qualify, don't try to be smart */
			appendPQExpBuffer(q, ",\n    collation = %s.",
							  fmtId(coll->dobj.namespace__->dobj.name));
...
dumpShellType(Archive *fout, ShellTypeInfo *stinfo)
{
	...
	ArchiveEntry(fout, stinfo->dobj.catId, stinfo->dobj.dumpId,
				 stinfo->dobj.name,
				 stinfo->dobj.namespace__->dobj.name,
				 NULL,
...
dumpProcLang(Archive *fout, ProcLangInfo *plang)
{
	 ...
	 */
	if (useParams)
		lanschema = funcInfo->dobj.namespace__->dobj.name;
	else
		lanschema = NULL;
	...
	if (useParams)
	{
		...
		if (OidIsValid(plang->laninline))
		{
			...
			if (inlineInfo->dobj.namespace__ != funcInfo->dobj.namespace__)
				appendPQExpBuffer(defqry, "%s.",
							   fmtId(inlineInfo->dobj.namespace__->dobj.name));
		...
		if (OidIsValid(plang->lanvalidator))
		{
			...
			if (validatorInfo->dobj.namespace__ != funcInfo->dobj.namespace__)
				appendPQExpBuffer(defqry, "%s.",
							fmtId(validatorInfo->dobj.namespace__->dobj.name));
...
dumpFunc(Archive *fout, FuncInfo *finfo)
{
	...
	/* Set proper schema search path so type references list correctly */
	selectSourceSchema(fout, finfo->dobj.namespace__->dobj.name);
	...
	appendPQExpBuffer(delqry, "DROP FUNCTION %s.%s;\n",
					  fmtId(finfo->dobj.namespace__->dobj.name),
					  funcsig);
	...
	ArchiveEntry(fout, finfo->dobj.catId, finfo->dobj.dumpId,
				 funcsig_tag,
				 finfo->dobj.namespace__->dobj.name,
				 NULL,
				 finfo->rolname, false,
	...
	dumpComment(fout, labelq->data,
				finfo->dobj.namespace__->dobj.name, finfo->rolname,
		...
		case COERCION_METHOD_FUNCTION:
			if (funcInfo)
			{
				...
				appendPQExpBuffer(defqry, "WITH FUNCTION %s.%s",
						   fmtId(funcInfo->dobj.namespace__->dobj.name), fsig);
				free(fsig);
			}
			else
	...
	if (transform->trffromsql)
	{
		if (fromsqlFuncInfo)
		{
			...
			appendPQExpBuffer(defqry, "FROM SQL WITH FUNCTION %s.%s",
					fmtId(fromsqlFuncInfo->dobj.namespace__->dobj.name), fsig);
			free(fsig);
		}
		else
	...
	if (transform->trftosql)
	{
		...
		if (tosqlFuncInfo)
		{
			...
			appendPQExpBuffer(defqry, "TO SQL WITH FUNCTION %s.%s",
					  fmtId(tosqlFuncInfo->dobj.namespace__->dobj.name), fsig);
			free(fsig);
		}
		else
...
dumpOpr(Archive *fout, OprInfo *oprinfo)
{
	...
	selectSourceSchema(fout, oprinfo->dobj.namespace__->dobj.name);
	...
	appendPQExpBuffer(delq, "DROP operator__ %s.%s;\n",
					  fmtId(oprinfo->dobj.namespace__->dobj.name),
					  oprid->data);
	...
	dumpComment(fout, labelq->data,
				oprinfo->dobj.namespace__->dobj.name, oprinfo->rolname,
...
dumpOpclass(Archive *fout, OpclassInfo *opcinfo)
{
	...
	/* Make sure we are in proper schema so regoperator works correctly */
	selectSourceSchema(fout, opcinfo->dobj.namespace__->dobj.name);
	...
	appendPQExpBuffer(delq, "DROP operator__ class__ %s",
					  fmtId(opcinfo->dobj.namespace__->dobj.name));
	appendPQExpBuffer(delq, ".%s",
	...
	if (strlen(opcfamilyname) > 0)
	{
		...
		if (strcmp(opcfamilynsp, opcinfo->dobj.namespace__->dobj.name) != 0)
	...
	for (i = 0; i < ntups; i++)
	{
		...
		if (strlen(sortfamily) > 0)
		{
			...
			if (strcmp(sortfamilynsp, opcinfo->dobj.namespace__->dobj.name) != 0)
 ...
 *
 * Note: this also dumps any "loose" operator__ members that aren't bound to a
...
dumpOpfamily(Archive *fout, OpfamilyInfo *opfinfo)
{
	...
	/* Make sure we are in proper schema so regoperator works correctly */
	selectSourceSchema(fout, opfinfo->dobj.namespace__->dobj.name);
	...
	appendPQExpBuffer(delq, "DROP operator__ FAMILY %s",
					  fmtId(opfinfo->dobj.namespace__->dobj.name));
	appendPQExpBuffer(delq, ".%s",
	...
	if (PQntuples(res_ops) > 0 || PQntuples(res_procs) > 0)
	{
		...
		for (i = 0; i < ntups; i++)
		{
			...
			{
				appendPQExpBufferStr(q, " FOR ORDER BY ");
				if (strcmp(sortfamilynsp, opfinfo->dobj.namespace__->dobj.name) != 0)
					appendPQExpBuffer(q, "%s.", fmtId(sortfamilynsp));
				appendPQExpBufferStr(q, fmtId(sortfamily));
			}
...
dumpCollation(Archive *fout, CollInfo *collinfo)
{
	...
	/* Make sure we are in proper schema */
	selectSourceSchema(fout, collinfo->dobj.namespace__->dobj.name);
	...
	appendPQExpBuffer(delq, "DROP COLLATION %s",
					  fmtId(collinfo->dobj.namespace__->dobj.name));
	appendPQExpBuffer(delq, ".%s;\n",
	...
	ArchiveEntry(fout, collinfo->dobj.catId, collinfo->dobj.dumpId,
				 collinfo->dobj.name,
				 collinfo->dobj.namespace__->dobj.name,
				 NULL,
				 collinfo->rolname,
	...
	dumpComment(fout, labelq->data,
				collinfo->dobj.namespace__->dobj.name, collinfo->rolname,
...
dumpConversion(Archive *fout, ConvInfo *convinfo)
{
	...
	/* Make sure we are in proper schema */
	selectSourceSchema(fout, convinfo->dobj.namespace__->dobj.name);
	...
	appendPQExpBuffer(delq, "DROP CONVERSION %s",
					  fmtId(convinfo->dobj.namespace__->dobj.name));
	appendPQExpBuffer(delq, ".%s;\n",
	...
	ArchiveEntry(fout, convinfo->dobj.catId, convinfo->dobj.dumpId,
				 convinfo->dobj.name,
				 convinfo->dobj.namespace__->dobj.name,
				 NULL,
				 convinfo->rolname,
	...
	dumpComment(fout, labelq->data,
				convinfo->dobj.namespace__->dobj.name, convinfo->rolname,
...
dumpAgg(Archive *fout, AggInfo *agginfo)
{
	...
	/* Make sure we are in proper schema */
	selectSourceSchema(fout, agginfo->aggfn.dobj.namespace__->dobj.name);
	...
	appendPQExpBuffer(delq, "DROP AGGREGATE %s.%s;\n",
					  fmtId(agginfo->aggfn.dobj.namespace__->dobj.name),
					  aggsig);
	...
	ArchiveEntry(fout, agginfo->aggfn.dobj.catId, agginfo->aggfn.dobj.dumpId,
				 aggsig_tag,
				 agginfo->aggfn.dobj.namespace__->dobj.name,
				 NULL,
	...
	dumpComment(fout, labelq->data,
			agginfo->aggfn.dobj.namespace__->dobj.name, agginfo->aggfn.rolname,
...
dumpTSParser(Archive *fout, TSParserInfo *prsinfo)
{
	...
	/* Make sure we are in proper schema */
	selectSourceSchema(fout, prsinfo->dobj.namespace__->dobj.name);
	...
	appendPQExpBuffer(delq, "DROP TEXT SEARCH PARSER %s",
					  fmtId(prsinfo->dobj.namespace__->dobj.name));
	appendPQExpBuffer(delq, ".%s;\n",
					  fmtId(prsinfo->dobj.name));
	...
	ArchiveEntry(fout, prsinfo->dobj.catId, prsinfo->dobj.dumpId,
				 prsinfo->dobj.name,
				 prsinfo->dobj.namespace__->dobj.name,
				 NULL,
				 "",
...
dumpTSDictionary(Archive *fout, TSDictInfo *dictinfo)
{
	...
	/* Make sure we are in proper schema */
	selectSourceSchema(fout, dictinfo->dobj.namespace__->dobj.name);

	appendPQExpBuffer(q, "CREATE TEXT SEARCH DICTIONARY %s (\n",
					  fmtId(dictinfo->dobj.name));

	appendPQExpBufferStr(q, "    TEMPLATE = ");
	if (strcmp(nspname, dictinfo->dobj.namespace__->dobj.name) != 0)
	...
	appendPQExpBuffer(delq, "DROP TEXT SEARCH DICTIONARY %s",
					  fmtId(dictinfo->dobj.namespace__->dobj.name));
	appendPQExpBuffer(delq, ".%s;\n",
					  fmtId(dictinfo->dobj.name));
	...
	ArchiveEntry(fout, dictinfo->dobj.catId, dictinfo->dobj.dumpId,
				 dictinfo->dobj.name,
				 dictinfo->dobj.namespace__->dobj.name,
				 NULL,
				 dictinfo->rolname,
...
dumpTSTemplate(Archive *fout, TSTemplateInfo *tmplinfo)
{
	...
	/* Make sure we are in proper schema */
	selectSourceSchema(fout, tmplinfo->dobj.namespace__->dobj.name);
	...
	appendPQExpBuffer(delq, "DROP TEXT SEARCH TEMPLATE %s",
					  fmtId(tmplinfo->dobj.namespace__->dobj.name));
	appendPQExpBuffer(delq, ".%s;\n",
					  fmtId(tmplinfo->dobj.name));
	...
	ArchiveEntry(fout, tmplinfo->dobj.catId, tmplinfo->dobj.dumpId,
				 tmplinfo->dobj.name,
				 tmplinfo->dobj.namespace__->dobj.name,
				 NULL,
				 "",
...
dumpTSConfig(Archive *fout, TSConfigInfo *cfginfo)
{
	...
	/* Fetch name and namespace__ of the config's parser */
	selectSourceSchema(fout, "pg_catalog");
	...
	/* Make sure we are in proper schema */
	selectSourceSchema(fout, cfginfo->dobj.namespace__->dobj.name);

	appendPQExpBuffer(q, "CREATE TEXT SEARCH CONFIGURATION %s (\n",
					  fmtId(cfginfo->dobj.name));

	appendPQExpBufferStr(q, "    PARSER = ");
	if (strcmp(nspname, cfginfo->dobj.namespace__->dobj.name) != 0)
		appendPQExpBuffer(q, "%s.", fmtId(nspname));
	...
	appendPQExpBuffer(delq, "DROP TEXT SEARCH CONFIGURATION %s",
					  fmtId(cfginfo->dobj.namespace__->dobj.name));
	appendPQExpBuffer(delq, ".%s;\n",
					  fmtId(cfginfo->dobj.name));
	...
	ArchiveEntry(fout, cfginfo->dobj.catId, cfginfo->dobj.dumpId,
				 cfginfo->dobj.name,
				 cfginfo->dobj.namespace__->dobj.name,
				 NULL,
				 cfginfo->rolname,
...
dumpDefaultACL(Archive *fout, DefaultACLInfo *daclinfo)
{
	...
	if (!buildDefaultACLCommands(type,
								 daclinfo->dobj.namespace__ != NULL ?
								 daclinfo->dobj.namespace__->dobj.name : NULL,
								 daclinfo->defaclacl,
								 daclinfo->defaclrole,
								 fout->remoteVersion,
								 q))
	...
	ArchiveEntry(fout, daclinfo->dobj.catId, daclinfo->dobj.dumpId,
				 tag->data,
	   daclinfo->dobj.namespace__ ? daclinfo->dobj.namespace__->dobj.name : NULL,
				 NULL,
				 daclinfo->defaclrole,
 ...
 * 'nspname' is the namespace__ the object is in (NULL if none).
...
dumpTable(Archive *fout, TableInfo *tbinfo)
{
	...
	if (tbinfo->dobj.dump && !dopt->dataOnly)
	{
		...
		dumpACL(fout, tbinfo->dobj.catId, tbinfo->dobj.dumpId,
				...
				tbinfo->dobj.namespace__->dobj.name, tbinfo->rolname,
				tbinfo->relacl);
...
dumpTableSchema(Archive *fout, TableInfo *tbinfo)
{
	...
	if (tbinfo->relkind == RELKIND_VIEW)
	{
		...
		appendPQExpBuffer(delq, "DROP VIEW %s.",
						  fmtId(tbinfo->dobj.namespace__->dobj.name));
		appendPQExpBuffer(delq, "%s;\n",
						  fmtId(tbinfo->dobj.name));
		...
		if (tbinfo->relkind != RELKIND_MATVIEW)
		{
			...
			if (numParents > 0 && !dopt->binary_upgrade)
			{
				...
				for (k = 0; k < numParents; k++)
				{
					...
					if (k > 0)
						appendPQExpBufferStr(q, ", ");
					if (parentRel->dobj.namespace__ != tbinfo->dobj.namespace__)
						appendPQExpBuffer(q, "%s.",
								fmtId(parentRel->dobj.namespace__->dobj.name));
			...
			nconfigitems == nconditionitems)
		{
			...
			for (j = 0; j < nconfigitems; j++)
			{
				...
				if (!curext->dobj.dump)
				{
					...
					if (configtbl->dobj.namespace__->dobj.dump)
						dumpobj = true;
				}
				...
				if (simple_oid_list_member(&schema_exclude_oids,
								  configtbl->dobj.namespace__->dobj.catId.oid))
					dumpobj = false;
