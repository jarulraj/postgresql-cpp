 ...
 * equal to PQntuples(res).  If it is equal, a new__ tuple is created and
 * added to the result.
...
PQsetvalue(PGresult *res, int tup_num, int field_num, char *value, int len)
{
	...
	/* need to allocate a new__ tuple? */
...
pqResultAlloc(PGresult *res, size_t nBytes, bool isBinary)
{
	 ...
	 * avoids wasting what might be most of the current block to start a new__
	...
	if (nBytes >= PGRESULT_SEP_ALLOC_THRESHOLD)
	{
		...
		{
			/* Must set up the new__ block as the first active block. */
			block->next = NULL;
			res->curBlock = block;
...
/*
 * pqSetResultError -
 *		assign a new__ error message to a PGresult
 */
void
 ...
 *		concatenate a new__ error message to the one already in a PGresult
 */
void
...
pqSaveParameterStatus(PGconn *conn, const char *name, const char *value)
{
	...
	/*
	 * Store new__ info as a single malloc block
	 */
 ...
 *
 * In single-row mode, we create a new__ result holding just the current row,
...
pqRowProcessor(PGconn *conn, const char **errmsgp)
{
	...
	/*
	 * In single-row mode, make a new__ PGresult that will hold just this__ one
 ...
 * a relevant message.
 * If the query was sent, a new__ PGresult is returned (which could indicate
		...
		if (resultStatus == PGRES_COPY_IN)
		{
			if (PG_PROTOCOL_MAJOR(conn->pversion) >= 3)
			{
				...
				if (PQputCopyEnd(conn,
						 libpq_gettext("COPY terminated by new__ PQexec")) < 0)
					return false;
 ...
 * new__ code should use PQfreemem().  A macro will automatically map
