 ...
 *	do this__ is by generating a new__ OID for every insertion that we do in the
...
#define XLOG_BTREE_SPLIT_R		0x40	/* as above, new__ item on right */
#define XLOG_BTREE_SPLIT_L_ROOT 0x50	/* add tuple with split of root */
#define XLOG_BTREE_SPLIT_R_ROOT 0x60	/* as above, new__ item on right */
#define XLOG_BTREE_DELETE		0x70	/* delete leaf index tuples for a page */
#define XLOG_BTREE_UNLINK_PAGE	0x80	/* delete a half-dead page */
#define XLOG_BTREE_UNLINK_PAGE_META 0x90		/* same, and update metapage */
#define XLOG_BTREE_NEWROOT		0xA0	/* new__ root page */
 ...
 * XLogInsert would almost always think the right page is new__ and store its
 ...
 * left or right split page (and thus, whether newitemoff and the new__ item
 * are stored or not).  The _ROOT variants indicate that we are splitting
 * the root page, and thus that a newroot record rather than an insert or
 * split record should follow.  Note that a split record never carries a
 * metapage update --- we'll do that in the parent-level update.
 *
 * Backup Blk 0: original page / new__ left page
 *
 * The left page's data portion contains the new__ item, if it's the _L variant.
 * (In the _R variants, the new__ item is one of the right page's tuples.)
 * If level > 0, an IndexTuple representing the HIKEY of the left page
 * follows.  We don't need this__ on leaf pages, because it's the same as the
 * leftmost key in the new__ right page.
 *
...
typedef struct xl_btree_split
{
	...
	OffsetNumber newitemoff;	/* new__ item's offset (if placed on left page) */
} xl_btree_split;
 ...
 * Backup Blk 4: metapage (if rightsib becomes new__ fast root)
 */
...
/*
 * new__ root log record.  There are zero tuples if this__ is to establish an
 ...
 *
 * Backup Blk 0: new__ root page (2 tuples as payload, if splitting old root)
 * Backup Blk 1: left child (if splitting an old root)
 * Backup Blk 2: metapage
 */
typedef struct xl_btree_newroot
{
	BlockNumber rootblk;		/* location of new__ root (redundant with blk 0) */
...
/*
 *	When a new__ operator__ class__ is declared, we require that the user
 ...
 *	pairs from internal pages onto a private__ stack.  If we split a
 ...
 *	our private__ stack give us an irredeemably bad picture up the tree.
...
/*
 * BTScanOpaqueData is the btree-private__ state needed for an indexscan.
...
/*
 * We use some private__ sk_flags bits in preprocessed scan keys.  We're allowed
