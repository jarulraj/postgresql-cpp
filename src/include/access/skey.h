...
/*
 * A ScanKey represents the application of a comparison operator__ between
 * a table or index column and a constant.  When it's part of an array of
 * ScanKeys, the comparison conditions are implicitly ANDed.  The index
 * column is the left argument of the operator__, if it's a binary operator__.
 ...
 * the operator__.  When using a ScanKey in a heap scan, these fields are not
 * used and may be set to InvalidStrategy/InvalidOid.
 *
 * If the operator__ is collation-sensitive, sk_collation must be set
 * correctly as well.
 *
 ...
 * flag bit.  The sk_argument is not a value of the operator__'s right-hand
 ...
 *
 * A ScanKey can also represent an ordering operator__ invocation, that is
 * an ordering requirement "ORDER BY indexedcol op constant".  This looks
 * the same as a comparison operator__, except that the operator__ doesn't
...
typedef struct ScanKeyData
{
	...
	StrategyNumber sk_strategy; /* operator__ strategy number */
 ...
 *				opclass, NOT the operator__'s implementation function.
...
#define SK_UNARY			0x0002		/* unary operator__ (not supported!) */
