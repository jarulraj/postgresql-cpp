...
typedef struct PlannedStmt
{
	...
	Node	   *utilityStmt;	/* non-null if this__ is DECLARE CURSOR */
 ...
 *
 * We never actually instantiate any Plan nodes; this__ is just the common
...
typedef struct Plan
{
	...
	/*
	 * planner's estimate of result size of this__ plan step
	 */
	 ...
	 * affecting this__ plan node or its children.  setParam params from the
	 * node's initPlans are not included, but their extParams are.
	 *
	 * allParam includes all the extParam paramIDs, plus the IDs of local
	 * params that affect the node (i.e., the setParams of its initplans).
	 * These are _all_ the PARAM_EXEC params that affect this__ node.
	 */
	Bitmapset  *extParam;
	Bitmapset  *allParam;
} Plan;
...
typedef struct ModifyTable
{
	...
	List	   *fdwPrivLists;	/* per-target-table FDW private__ data lists */
 ...
 * must appear in exactly the ORDER BY order, and this__ is not necessarily the
 ...
 * Because of this__, *all* Vars in the plan node's targetlist, qual, and
 ...
 * with one TLE per index column.  Vars appearing in this__ list reference
 * the base table, and this__ is the only field in the plan node that may
 * contain such Vars.
 * ----------------
 */
 ...
 * relation, we make this__ a descendant of Scan anyway for code-sharing
 * purposes.
 *
...
typedef struct ForeignScan
{
	...
	List	   *fdw_private;	/* private__ data for FDW */
	List	   *fdw_scan_tlist; /* optional tlist describing scan tuple */
	List	   *fdw_recheck_quals;	/* original quals not in scan.plan.qual */
	Bitmapset  *fs_relids;		/* RTIs generated by this__ scan */
...
typedef struct CustomScan
{
	...
	List	   *custom_private; /* private__ data for custom code */
	List	   *custom_scan_tlist;		/* optional tlist describing scan
										 * tuple */
	Bitmapset  *custom_relids;	/* RTIs generated by this__ scan */
 ...
 * same way according to the common opfamily and collation.  The operator__ in
 * each mergeclause must be an equality operator__ of the indicated opfamily.
 * ----------------
 */
 ...
 * this__ could happen if they get optimized away by constant-folding, or if
 ...
 *		lock-rows node
 *
 * rowMarks identifies the rels to be locked by this__ node; it should be
 * a subset of the rowMarks listed in the top-level PlannedStmt.
 * epqParam is a Param that all scan nodes below this__ one must depend on.
 ...
 * can just fetch the TID, much as for a target relation; this__ case is
 ...
 * (Again, this__ probably doesn't make sense if a physical remote fetch is
 ...
 * of (1<<markType) across all its children (this__ definition allows children
 ...
 * Note this__ means that all tables in an inheritance hierarchy share the
...
typedef struct PlanRowMark
{
	...
	bool		isParent;		/* true if this__ is a "dummy" parent entry */
} PlanRowMark;
