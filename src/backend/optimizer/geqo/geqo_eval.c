...
gimme_tree(PlannerInfo *root, Gene *tour, int num_gene)
{
	 ...
	 * new__ relation from the tour is added to the first clump it can be joined
	 * to; if there is none then it becomes a new__ clump of its own. When we
	...
	for (rel_count = 0; rel_count < num_gene; rel_count++)
	{
		...
		cur_rel = (RelOptInfo *) list_nth(private__->initial_rels,
										  cur_rel_index - 1);
