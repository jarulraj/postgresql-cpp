...
inheritance_planner(PlannerInfo *root)
{
	...
	foreach(lc, root->append_rel_list)
	{
		...
		else
		{
			 ...
			 *
			 * When this__ happens, we want to use the new__ subqueries in the
			 * final rtable.
			 */
...
grouping_planner(PlannerInfo *root, double tuple_fraction)
{
	...
	else
	{
		 ...
		 * This may add new__ security barrier subquery RTEs to the rangetable.
		 */
	...
	if (parse->rowMarks)
	{
		 ...
		 * cause the sort key columns to be replaced with new__ values.
		 */
		current_pathkeys = NIL;
	}
 ...
 * can add on later, only Agg, WindowAgg, and Group project new__ tlists (the
...
preprocess_groupclause(PlannerInfo *root, List *force)
{
	...
	/*
	 * Add any remaining GROUP BY items to the new__ list, but only if we were
 ...
 * new__ tlist to evaluate the resjunk columns.  For now, just ereport if we
