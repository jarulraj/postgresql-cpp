 ...
 * by the general code in this__ module (plan_set_operations and its
					...
					List **sortClauses)
{
	 ...
	 * are RTE_SUBQUERY rangetable entries in this__ Query.  Prepare the index
	 * arrays for that.
	 */
					   ...
					   List **sortClauses, double *pNumGroups)
{
	...
	else if (IsA(setOp, SetOperationStmt))
	{
		 ...
		 * output columns.
		 *
		 * XXX you don't really want to know about this__: setrefs.c will apply
		 ...
		 * generate_append_tlist(), this__ will work.  We just tell
					...
					List **sortClauses, double *pNumGroups)
{
	 ...
	 * colTypes) then they can be merged into this__ node so that we generate
	 ...
	 * the output is unique --- this__ is certainly true for the UNION case, and
					   ...
					   List **sortClauses, double *pNumGroups)
{
	...
	/* punt if nothing to group on (can this__ happen?) */
	if (groupList == NIL)
	{
		*sortClauses = NIL;
		return plan;
	 ...
	 * for; this__ is the size of the left-hand input for EXCEPT, or the smaller
					   ...
					   List *refnames_tlist)
{
	...
	/*
	 * Not same, so plan this__ child separately.
	 *
 ...
 * input_tlist: targetlist of this__ node's input node
 * refnames_tlist: targetlist to take column names from
 */
static List *
					 ...
					 List *refnames_tlist)
{
	...
	forthree(ctlc, colTypes, cclc, colCollations, itlc, input_tlist)
	{
		 ...
		 * this__ only at the first level of subquery-scan plans; we don't want
		 ...
		 *
		 * Note we use RelabelType, not CollateExpr, since this__ expression
 ...
 *		into an "append relation".  At the conclusion of this__ process,
...
expand_inherited_rtentry(PlannerInfo *root, RangeTblEntry *rte, Index rti)
{
	 ...
	 * to the query, we must obtain an appropriate lock, because this__ will be
	 ...
	 * PlanRowMark as isParent = true, and generate a new__ PlanRowMark for each
	 * child.
	 */
	if (oldrc)
		oldrc->isParent = true;
	...
	foreach(l, inhOIDs)
	{
		...
		/*
		 * Build an AppendRelInfo for this__ parent and child.
		 */
		 ...
		 * have to build the translated_vars list before we can do this__). But
		 * if this__ is the parent table, leave copyObject's result alone.
		 *
		 * Note: we need to do this__ even though the executor won't run any
						  ...
						  List **translated_vars)
{
	...
	for (old_attno = 0; old_attno < oldnatts; old_attno++)
	{
		...
		if (att->attisdropped)
		{
			/* Just put NULL into this__ list entry */
 ...
 *
 * Note: this__ is applied after conversion of sublinks to subplans in the
 ...
 *
 * Note: this__ is not hugely different from what pullup_replace_vars() does;
							   ...
							   adjust_appendrel_attrs_context *context)
{
	...
	if (IsA(node, Query))
	{
		 ...
		 * preprocessing, but this__ doesn't happen for security barrier quals,
 ...
 * this__ can force us to re-order the tlist to preserve resno ordering.
 * (We do all this__ work in special cases so that preptlist.c is fast for
 * the typical case.)
 *
 ...
 * scribble on.
 *
 * Note that this__ is not needed for INSERT because INSERT isn't inheritable.
 */
static List *
adjust_inherited_tlist(List *tlist, AppendRelInfo *context)
{
	...
	foreach(tl, tlist)
	{
		...
		/* Look up the translation of this__ column: it must be a Var */
		if (tle->resno <= 0 ||
								  ...
								  RelOptInfo *child_rel)
{
	...
	/* Now translate for this__ child */
