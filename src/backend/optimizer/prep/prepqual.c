		...
		case T_BoolExpr:
			{
				...
				switch (expr->boolop)
				{
						 ...
						 * building a new__ OR clause.  Similarly for the OR case.
						 *--------------------
						 */
					case AND_EXPR:
						{
...
pull_ands(List *andlist)
{
	...
	foreach(arg, andlist)
	{
		 ...
		 * built a new__ arglist not shared with any other expr. Otherwise we'd
...
process_duplicate_ors(List *orlist)
{
	 ...
	 * clauses.  Build a new__ list of winning clauses.
	 */
	winners = NIL;
	...
	/*
	 * Generate new__ OR list consisting of the remaining sub-clauses.
	 *
