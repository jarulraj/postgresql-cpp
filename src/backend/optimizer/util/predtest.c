...
arrayconst_startup_fn(Node *clause, PredIterInfo info)
{
	...
	state->constexpr__.xpr.type = T_Const;
	state->constexpr__.consttype = ARR_ELEMTYPE(arrayval);
	state->constexpr__.consttypmod = -1;
	state->constexpr__.constcollid = arrayconst->constcollid;
	state->constexpr__.constlen = elmlen;
	state->constexpr__.constbyval = elmbyval;
	lsecond(state->opexpr.args) = &state->constexpr__;
...
arrayconst_next_fn(PredIterInfo info)
{
	...
	if (state->next_elem >= state->num_elems)
		return NULL;
	state->constexpr__.constvalue = state->elem_values[state->next_elem];
	state->constexpr__.constisnull = state->elem_nulls[state->next_elem];
	state->next_elem++;
...
operator_predicate_proof(Expr *predicate, Node *clause, bool refute_it)
{
	 ...
	 *
	 * Note: in future we might extend this logic to other operator__-based
