...
count_agg_clauses_walker(Node *node, count_agg_clauses_context *context)
{
	...
	if (IsA(node, Aggref))
	{
		...
		else if (aggtranstype == INTERNALOID)
		{
			 ...
			 * being kept in a private__ memory context, as is done by
...
contain_leaked_vars_walker(Node *node, void *context)
{
	...
	switch (nodeTag(node))
	{
		...
		default:
			 ...
			 * This prevents an unexpected security hole if someone adds a new__
		...
		case T_CaseExpr:
			{
				...
				foreach(arg, caseexpr->args)
				{
					...
					/* If non-constant test condition, emit a new__ WHEN node */
					if (!const_true_cond)
					{
				...
				if (newargs == NIL)
					return defresult;
				/* Otherwise we need a new__ CASE node */
...
inline_set_returning_function(PlannerInfo *root, RangeTblEntry *rte)
{
	...
	/* copy up any new__ invalItems, too */
