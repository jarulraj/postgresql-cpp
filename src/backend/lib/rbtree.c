 ...
 *	combiner: merge an existing tree entry with a new__ one
 *	allocfunc: allocate a new__ RBNode
...
/*
 * rb_insert: insert a new__ value into the tree.
 *
 ...
 * we copy "data" into a new__ tree entry and return that node, setting *isNew
 * to true.
 *
...
rb_insert(RBTree *rb, const RBNode *data, bool *isNew)
{
	...
	/*
	 * Value is not present, so create a new__ node containing data.
	 */
	*isNew = true;
