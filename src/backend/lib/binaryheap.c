 ...
 * to a new__ heap. To obtain a valid heap, one must call binaryheap_build()
 * afterwards.
 */
void
...
binaryheap_remove_first(binaryheap *heap)
{
	 ...
	 * remove the former root node) and sift the new__ root node down to its
	 * correct position.
	 */
