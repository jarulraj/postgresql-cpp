 ...
 * with those of the new__ tuple.  If the tuple values are not consistent with
...
brininsert(PG_FUNCTION_ARGS)
{
	...
	for (;;)
	{
		...
		/*
		 * Compare the key values of the new__ tuple to the stored index values;
		 * our deformed tuple will get updated if the new__ tuple doesn't fit
		...
		{
			/*
			 * The tuple is consistent with the new__ values, so there's nothing
			 * to do.
			 */
		...
		else
		{
			 ...
			 * recompute to ensure both our new__ heap tuple and the other
...
/*
 * brinbuild() -- build a new__ BRIN index.
 */
Datum
				...
				BlockNumber heapBlk, BlockNumber heapNumBlks)
{
	 ...
	 * short of brinbuildCallback creating the new__ index entry.
	 *
	...
	for (;;)
	{
		 ...
		 * placeholder tuple concurrently.  Extract the new__ version, union it
 ...
 *
 * For each new__ index tuple inserted, *numSummarized (if not NULL) is
...
brin_vacuum_scan(Relation idxrel, BufferAccessStrategy strategy)
{
	...
	/*
	 * If we made any change to the FSM, make sure the new__ info is visible all
	 * the way to the top.
	 */
	if (vacuum_fsm)
