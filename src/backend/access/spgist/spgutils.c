...
/*
 * Allocate and initialize a new__ buffer of the type and parity specified by
...
SpGistGetBuffer(Relation index, int flags, int needSpace, bool *isNew)
{
	...
	if (lup->freeSpace >= needSpace)
	{
		...
		{
			/*
			 * buffer is locked by another process, so return a new__ buffer
			 */
			ReleaseBuffer(buffer);
			*isNew = true;
			return allocNewBuffer(index, flags);
		...
		/*
		 * fallback to allocation of new__ buffer
		 */
		UnlockReleaseBuffer(buffer);
	}

	/* No success with cache, so return a new__ buffer */
	*isNew = true;
	return allocNewBuffer(index, flags);
}
...
/*
 * Add a new__ item to the page, replacing a PLACEHOLDER item if possible.
