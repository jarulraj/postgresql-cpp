 ...
 * parent level (starting a new__ parent level if necessary).  When
...
/*
 * allocate workspace for a new__, clean btree page, not linked to any siblings.
 */
static Page
...
/*
 * allocate and initialize a new__ BTPageState.  the returned structure
...
_bt_buildadd(BTWriteState *wstate, BTPageState *state, IndexTuple itup)
{
	...
	if (pgspc < itupsz || (pgspc < state->btps_full && last_off > P_FIRSTKEY))
	{
		...
		/* Create new__ page of same level */
		...
		/*
		 * We copy the last item on the page into the new__ page, and then
		...
		*hii = *ii;
		...
		/*
		 * Save a copy of the minimum key for the new__ page.  We have to copy
		 * it off the old page, not the new__ one, in case we are not at leaf
		 * level.
		 */
		...
		/*
		 * Reset last_off to point to new__ page
		 */
		last_off = P_FIRSTKEY;
	}
	...
	/*
	 * Add the new__ item into the current page.
	 */
...
_bt_uppershutdown(BTWriteState *wstate, BTPageState *state)
{
	 ...
	 * point to the new__ root (unless we had no data at all, in which case it's
