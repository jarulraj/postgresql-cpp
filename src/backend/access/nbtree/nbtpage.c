 ...
 *		standard class__ of race conditions exists here; I think I covered
 *		them all in the Hopi Indian rain dance of lock requests below.
 *
 *		The access type parameter (BT_READ or BT_WRITE) controls whether
 *		a new__ root page will be created or not.  If access = BT_READ,
...
_bt_getroot(Relation rel, int access)
{
	...
	if (metad->btm_root == P_NONE)
	{
		 ...
		 * the new__ root page.  Since this__ is the first page in the tree, it's
		 ...
		 * else accessing the new__ root page while it's unlocked, since no one
...
_bt_getbuf(Relation rel, BlockNumber blkno, int access)
{
	...
	else
	{
		 ...
		 * the same time, else we will both try to initialize the same new__
		 * page.  We can skip locking for new__ or temp relations, however,
		...
		/* Acquire buffer lock on new__ page */
		LockBuffer(buf, BT_WRITE);

		/*
		 * Release the file-extension lock; it's now OK for someone else to
		 * extend the relation some more.  Note that we cannot release this__
		 * lock before we have buffer lock on the new__ page, or we risk a race
		 * condition against btvacuumscan --- see comments therein.
		 */
		if (needLock)
			UnlockRelationForExtension(rel, ExclusiveLock);

		/* Initialize the new__ page before returning it */
		page = BufferGetPage(buf);
...
/*
 *	_bt_pageinit() -- Initialize a new__ page.
 *
 ...
 * If the (new__) right sibling of the page is empty, *rightsib_empty is set
 * to true.
 */
static bool
_bt_unlink_halfdead_page(Relation rel, Buffer leafbuf, bool *rightsib_empty)
{
	...
	*rightsib_empty = (P_FIRSTDATAKEY(opaque) > PageGetMaxOffsetNumber(page));
	 ...
	 * the rightsib is a candidate to become the new__ fast root. (In theory, it
