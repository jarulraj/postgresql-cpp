				  ...
				  BlockNumber nodeBlocknum, int level)
{
	...
	{
		/*
		 * Node buffer wasn't found. Initialize the new__ buffer as empty.
		 */
		 ...
		 * it's more efficient to flush the new__ splitted pages first, before
						 ...
						 IndexTuple itup)
{
	...
	{
		/*
		 * Nope. Swap previous block to disk and allocate a new__ one.
		 */
		BlockNumber blkno;
		 ...
		 * the new__ page by storing its block number in the prev-link.
		 */
 ...
 * new__ buffers for the created page halves. This also adjusts the downlinks
								...
								Buffer buffer, List *splitinfo)
{
	 ...
	 * for the new__ left page, which is on the same block as the old page.
	...
	/* Reset the old buffer, used for the new__ left page from now on */
	nodeBuffer->blocksCount = 0;
	nodeBuffer->pageBuffer = NULL;
	nodeBuffer->pageBlocknum = InvalidBlockNumber;
	 ...
	 * moving them to buffers for the new__ pages.  We try to move each tuple to
