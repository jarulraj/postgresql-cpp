...
gistgetadjusted(Relation r, IndexTuple oldtup, IndexTuple addtup, GISTSTATE *giststate)
{
	...
	for (i = 0; i < r->rd_att->natts; i++)
	{
		...
		if (neednew)
			/* we already need new__ key, so we can skip check */
			continue;
 ...
 * of the new__ index key contained in "it".
 *
		   ...
		   GISTSTATE *giststate)
{
	 ...
	 * to the old best, or use the new__ one.  Once we decide to stick to the
	...
	for (i = FirstOffsetNumber; i <= maxoff; i = OffsetNumberNext(i))
	{
		...
		for (j = 0; j < r->rd_att->natts; j++)
		{
			...
			{
				/*
				 * new__ best penalty for column.  Tentatively select this__ tuple
				...
				/* we have new__ best, so reset keep-it decision */
				keep_current_best = -1;
			}
		...
		if (j == r->rd_att->natts && result != i)
		{
			...
			{
				/* we choose to use the new__ tuple */
				result = i;
 ...
 * returns new__ IndexTuple that contains GISTENTRY with fetched data
 */
IndexTuple
...
/*
 * Allocate a new__ page (either by recycling, or by extending the index file)
 *
