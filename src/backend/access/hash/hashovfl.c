...
_hash_addovflpage(Relation rel, Buffer metabuf, Buffer buf)
{
	...
	/* now that we have correct backlink, initialize new__ overflow page */
...
_hash_getovflpage(Relation rel, Buffer metabuf)
{
	 ...
	 * First, check to see if we have to add a new__ bitmap page too.
	 */
	if (last_bit == (uint32) (BMPGSZ_BIT(metap) - 1))
	{
		/*
		 * We create the new__ bitmap page with all pages marked "in use".
		 * Actually two pages in the new__ bitmap's range will exist
	...
	/* Calculate address of the new__ overflow page */
	bit = metap->hashm_spares[splitnum];
 ...
 *	_hash_initbitmap()
 *
 *	 Initialize a new__ bitmap page.  The metapage has a write-lock upon
 *	 entering the function, and must be written by caller after return.
 *
 * 'blkno' is the block number of the new__ bitmap page.
 *
 * All bits in the new__ bitmap page are set to "1", indicating "in use".
 */
void
				 ...
				 ForkNumber forkNum)
{
	...
	/*
	 * It is okay to write-lock the new__ bitmap page while holding metapage
	 * write lock, because no one else could be contending for the new__ page.
	...
	/* write out the new__ bitmap page (releasing write lock and pin) */
	_hash_wrtbuf(rel, buf);

	/* add the new__ bitmap page to the metapage's list of bitmaps */
