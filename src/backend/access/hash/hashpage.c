 ...
 * index is local to the current backend (ie, either temp or new__ in the
...
/*
 *	_hash_getnewbuf() -- Get a new__ page at the end of the index.
 *
...
/*
 *	_hash_pageinit() -- Initialize a new__ hash index page.
 */
void
...
/*
 * Attempt to expand the hash table by creating one new__ bucket.
 *
...
_hash_expandtable(Relation rel, Buffer metabuf)
{
	...
	/*
	 * Likewise lock the new__ bucket (should never fail).
	 *
	 * Note: it is safe to compute the new__ bucket's blkno here, even though we
	 * may still need to update the BUCKET_TO_BLKNO mapping.  This is because
	 * the current value of hashm_spares[hashm_ovflpoint] correctly shows
	 * where we are going to put a new__ splitpoint's worth of buckets.
	 */
	start_nblkno = BUCKET_TO_BLKNO(metap, new_bucket);

	if (_hash_has_active_scan(rel, new_bucket))
		elog(ERROR, "scan in progress on supposedly new__ bucket");

	if (!_hash_try_getlock(rel, start_nblkno, HASH_EXCLUSIVE))
		elog(ERROR, "could not get lock on supposedly new__ bucket");

	/*
	 * If the split point is increasing (hashm_maxbucket's log base 2
	 * increases), we need to allocate a new__ batch of bucket pages.
	 */
	...
	/* Relocate records to the new__ bucket */
...
/*
 * _hash_alloc_buckets -- allocate a new__ splitpoint's worth of bucket pages
 *
 ...
 * belong in the new__ bucket, and compress out any free space in the old
 * bucket.
 *
				  ...
				  uint32 lowmask)
{
	...
	/* initialize the new__ bucket's primary page */
	 ...
	 * new__ bucket, advancing along the old bucket's overflow bucket chain and
	 * adding overflow pages to the new__ bucket as needed.  Outer loop iterates
	...
	for (;;)
	{
			 ...
			 ooffnum = OffsetNumberNext(ooffnum))
		{
			...
			{
				/*
				 * insert the tuple into the new__ bucket.  if it doesn't fit on
				 * the current page in the new__ bucket, we must allocate a new__
				 * overflow page and place the tuple on that page instead.
				 *
				 * XXX we have a problem here if we fail to get space for a
				 * new__ overflow page: we'll error out leaving the bucket split
				...
				/*
				 * Insert tuple on new__ page, using _hash_pgaddtup to ensure
				 ...
				 * the new__ page and qsort them before insertion.
				 */
	 ...
	 * packed as tightly as possible.  The new__ bucket is already tight.
	 */
