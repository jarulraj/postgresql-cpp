 ...
 * a new__ backend beginning a seqscan to begin it close to where other backends
 ...
 * start new__ scans close to where the previous scan(s) are.  We don't try to
 ...
 * means more traversing of the LRU list when starting a new__ scan.
 */
 ...
 * there's no guarantee that the new__ scan will read the page before it leaves
...
ss_report_location(Relation rel, BlockNumber location)
{
	 ...
	 * Missing a few updates isn't critical, it just means that a new__ scan
