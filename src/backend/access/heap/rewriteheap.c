 ...
 * INTERFACE
 *
 * The caller is responsible for creating the new__ heap, all catalog
 * changes, supplying the tuples to be written to the new__ heap, and
 ...
 * end_heap_rewrite
 *
 * The contents of the new__ relation shouldn't be relied on until after
 ...
 * ones, if we just copied t_ctid fields to the new__ table the links would
 ...
 * fix the ctid to point to the new__ location of B, and insert both A and B
 * to the new__ heap.
 *
 * If we encounter B first, we can insert B to the new__ heap right away.
 * We then add an entry to the old_new_tid_map hash table showing B's
 * original tid (in the old heap) and new__ tid (in the new__ heap).
 * When we later encounter A, we get the new__ location of B from the table,
 ...
 *
 * We can't use the normal heap_insert function to insert into the new__
 ...
 * exclusive access to the heap.  raw_heap_insert builds new__ pages in
 ...
 * directly through smgr.  Note, however, that any data sent to the new__
...
typedef struct
{
	...
	ItemPointerData new_tid;	/* where we put it in the new__ heap */
} OldToNewMappingData;
...
{
	LogicalRewriteMappingData map;		/* map between old and new__ location of
										 * the tuple */
	dlist_node	node;
 ...
 * new_heap		new__, locked heap relation to insert tuples to
 * oldest_xmin	xid used by the caller to determine which tuples are dead
 * freeze_xid	xid before which tuples will be frozen
 * min_multi	multixact before which multis will be removed
 * use_wal		should the inserts to the new__ heap be WAL-logged?
 *
				   ...
				   bool use_wal)
{
	...
	state->rs_old_new_tid_map =
		hash_create("Rewrite / Old to new__ tid map",
					128,		/* arbitrary initial size */
					&hash_ctl,
					HASH_ELEM | HASH_BLOBS | HASH_CONTEXT);
 ...
 * new_tuple	new__, rewritten tuple to be inserted to new__ heap
 */
void
				   ...
				   HeapTuple old_tuple, HeapTuple new_tuple)
{
	...
	/*
	 * If the tuple has been updated, check the old-to-new__ mapping hash table.
	 */
							...
							&(old_tuple->t_data->t_ctid))))
	{
		...
		if (mapping != NULL)
		{
			 ...
			 * set the ctid of this__ tuple to point to the new__ location, and
	 ...
	 * in any new__ or known pair.  When we resolve a known pair, we will be
	...
	for (;;)
	{
								   ...
								   state->rs_oldest_xmin))
		{
			...
			if (unresolved != NULL)
			{
				 ...
				 * to, fix its t_ctid and insert it to the new__ heap.
				 */
				if (free_new)
			...
			{
				/*
				 * Remember the new__ tid of this__ tuple. We'll use it to set the
 ...
 * copied to the new__ table, but we still make note of them so that we
...
/*
 * Insert a tuple to the new__ relation.  This has to track heap_insert
 * and its subsidiary functions!
 *
 * t_self of the tuple is set to the new__ TID of the tuple. If t_ctid of the
 * tuple is invalid on entry, it's replaced with the new__ TID as well (in
...
raw_heap_insert(RewriteState state, HeapTuple tup)
{
	...
	/* If heaptup is a private__ copy, release it. */
	if (heaptup != tup)
 ...
 * from the old to the new__ location.
 *
...
logical_heap_rewrite_flush_mappings(RewriteState state)
{
	...
	while ((src = (RewriteMappingFile *) hash_seq_search(&seq_status)) != NULL)
	{
		...
		/* this__ file hasn't got any new__ mappings */
...
/*
 * Log a single (old->new__) mapping for 'xid'.
 */
static void
...
CheckPointLogicalRewriteHeap(void)
{
	...
	/*
	 * We start of with a minimum of the last redo pointer. No new__ decoding
