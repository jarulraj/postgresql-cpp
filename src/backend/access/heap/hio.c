 ...
 *	if HEAP_INSERT_SKIP_FSM is specified, we just append a new__ empty page to
 *	the end of the relation if the tuple won't fit on the current target page.
 *	This can save some cycles when we know the relation is new__ and doesn't
						  ...
						  Buffer *vmbuffer, Buffer *vmbuffer_other)
{
	 ...
	 * same time, else we will both try to initialize the same new__ page.  We
	 * can skip locking for new__ or temp relations, however, since no one else
	...
	/*
	 * Now acquire lock on the new__ page.
	 */
	LockBuffer(buffer, BUFFER_LOCK_EXCLUSIVE);
