 ...
 * toast_insert_or_update -
 *
 *	Delete no-longer-used toast-entries and create new__ ones to
 *	make the new__ tuple fit on INSERT or UPDATE
 *
 * Inputs:
 *	newtup: the candidate new__ tuple to be inserted
					   ...
					   int options)
{
	...
	for (i = 0; i < numAttrs; i++)
	{
		...
		{
			/*
			 * For INSERT simply get the new__ value
			 */
			new_value = (struct varlena *) DatumGetPointer(toast_values[i]);
		}
	...
	/*
	 * In the case we toasted any values, we need to build a new__ heap tuple
	...
	if (need_change)
	{
		...
		/*
		 * Calculate the new__ size of the tuple.
		 *
				 ...
				 struct varlena * oldexternal, int options)
{
	 ...
	 * conflict with either new__ or existing toast value OIDs.
	 */
	if (!OidIsValid(rel->rd_toastoid))
	{
	...
	else
	{
		...
		{
			/*
			 * new__ value; must choose an OID that doesn't conflict in either
			 * old or new__ toast table
			 */
			do
			{
				toast_pointer.va_valueid =
