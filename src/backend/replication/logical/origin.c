 ...
 *	 safely and conflict free assign new__ origins using a dirty snapshot.
 *
...
replorigin_create(char *roname)
{
	 ...
	 * to the exclusive lock there's no danger that new__ rows can appear while
	 * we're checking.
	 */
	...
	for (roident = InvalidOid + 1; roident < PG_UINT16_MAX; roident++)
	{
		...
		{
			/*
			 * Ok, found an unused roident, insert the new__ row and do a CCI,
				   ...
				   bool go_backward, bool wal_log)
{
	...
	/* Lock exclusively, as we may have to create a new__ table entry. */
	LWLockAcquire(ReplicationOriginLock, LW_EXCLUSIVE);
