...
/*
 * Create a new__ decoding context, for a new__ logical slot.
 *
						  ...
						  LogicalOutputPluginWriterWrite do_write)
{
	...
	while (true)
	{
		 ...
		 * the new__ restart_lsn above, so normally we should never need to loop
		 * more than twice.
		 */
	 ...
	 * date.
	 *
	 * So we have to acquire the ProcArrayLock to prevent computation of new__
	 * xmin horizons by other backends, get the safe decoding xid, and inform
	 * the slot machinery about the new__ limit. Once that's done the
...
/*
 * Create a new__ decoding context, for a logical slot that has previously been
 * used already.
 *
...
DecodingContextFindStartpoint(LogicalDecodingContext *ctx)
{
	...
	for (;;)
	{
		...
		/* the read_page callback waits for new__ WAL */
...
LogicalIncreaseRestartDecodingForSlot(XLogRecPtr current_lsn, XLogRecPtr restart_lsn)
{
	...
	if (slot->candidate_restart_valid == InvalidXLogRecPtr)
	{
		...
		elog(DEBUG1, "got new__ restart lsn %X/%X at %X/%X",
...
LogicalConfirmReceivedLocation(XLogRecPtr lsn)
{
		...
		MyReplicationSlot->candidate_restart_valid != InvalidXLogRecPtr)
	{
			...
			slot->candidate_xmin_lsn <= lsn)
		{
			 ...
			 * ->effective_xmin once the new__ state is synced to disk. After a
		...
		/* first write new__ xmin to disk, so we know whats up after a crash */
		...
		/*
		 * Now the new__ xmin is safely on disk, we can let the global value
