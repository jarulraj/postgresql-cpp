 ...
 * walsender.c
 *
 * The WAL sender process (walsender) is new__ as of Postgres 9.0. It takes
...
StartReplication(StartReplicationCmd *cmd)
{
	...
	if (cmd->timeline != 0)
	{
		...
		else
		{
			 ...
			 * segment that contains switchpoint, but on the new__ timeline, so
...
/*
 * Create a new__ replication slot.
 */
static void
...
WalSndWaitForWal(XLogRecPtr loc)
{
	...
	for (;;)
	{
		...
		/* Waiting for new__ WAL. Since we need to wait, we're now caught up. */
		WalSndCaughtUp = true;
...
/* compute new__ replication slot xmin horizon if needed */
static void
...
retry:
	...
	while (nbytes > 0)
	{
		...
		if (sendFile < 0 || !XLByteInSeg(recptr, sendSegNo))
		{
			 ...
			 * used portion of the old segment is copied to the new__ file.
			 *-------
			 */
	 ...
	 * already have been overwritten with new__ WAL records.
	 */
...
XLogSendPhysical(void)
{
	...
	else if (am_cascading_walsender)
	{
		...
		if (!RecoveryInProgress())
		{
			 ...
			 * ThisTimeLineID to the new__ current timeline.
			 */
		...
		if (becameHistoric)
		{
			 ...
			 * timeline history file of the new__ timeline to see where exactly
