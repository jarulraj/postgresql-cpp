...
/*
 * Initialize state for a new__ group of input values.
 */
static inline void
...
setop_retrieve_direct(SetOpState *setopstate)
{
	...
	{
		/*
		 * If we don't already have the first tuple of the new__ group, fetch it
		 * from the outer plan.
		 */
...
setop_fill_hash_table(SetOpState *setopstate)
{
	...
	for (;;)
	{
		...
		if (flag == firstFlag)
		{
			...
			/* If new__ tuple group, initialize counts */
			if (isnew)
