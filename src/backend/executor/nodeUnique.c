...
ExecUnique(UniqueState *node)
{
	...
	for (;;)
	{
		...
		/*
		 * Else test if the new__ tuple and the previously returned tuple match.
		 * If so then we loop back and fetch another new__ tuple from the
		 * subplan.
		 */
	...
	/*
	 * We have a new__ tuple different from the previous saved tuple (if any).
