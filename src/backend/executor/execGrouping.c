 ...
 *		execTuplesUnequal, given an array of equality operator__ OIDs.
 *
 ...
 *
 * If isnew is NULL, we do not create new__ entries; we return NULL if no
 * match is found.
 *
 * If isnew isn't NULL, then a new__ entry is created if no existing entry
 * matches.  On return, *isnew is true if the entry is newly created,
 * false if it existed already.  Any extra space in a new__ entry has been
 * zeroed.
 */
TupleHashEntry
					 ...
					 bool *isnew)
{
	...
	if (isnew)
	{
		...
		else
		{
			 ...
			 * "key data" dynahash.c copied into the new__ entry, but we don't
