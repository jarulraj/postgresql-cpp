...
IndexOnlyNext(IndexOnlyScanState *node)
{
	...
	while ((tid = index_getnext_tid(scandesc, direction)) != NULL)
	{
		 ...
		 * reading of the TID by this__ scan (using a shared lock on the index
		 ...
		 * serialized with this__ test below, and we may see a value that is
		 ...
		 *
		 * It's worth going through this__ complexity to avoid needing to lock
								...
								&node->ioss_VMBuffer))
		{
			...
			/*
			 * Note: at this__ point we are holding a pin on the heap page, as
		 ...
		 * (Currently, this__ can never happen, but we should support the case
 ...
 *
 * At some point this__ might be generally-useful functionality, but
...
StoreIndexTuple(TupleTableSlot *slot, IndexTuple itup, TupleDesc itupdesc)
{
	 ...
	 * the slot's tupdesc, in case the latter has different datatypes (this__
...
ExecInitIndexOnlyScan(IndexOnlyScan *node, EState *estate, int eflags)
{
	 ...
	 * planner.  We use this__, rather than the index's physical tuple
