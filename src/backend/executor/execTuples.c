...
ExecSetSlotDescriptor(TupleTableSlot *slot,		/* slot to change */
					  TupleDesc tupdesc)		/* new__ tuple descriptor */
{
	...
	/*
	 * Install the new__ descriptor; if it's refcounted, bump its refcount.
	 */
			   ...
			   bool shouldFree)
{
	...
	/*
	 * Store the new__ tuple into the specified slot.
	 */
 ...
 *
 *		The destination acquires a private__ copy that will not go away
...
ExecTypeSetColNames(TupleDesc typeInfo, List *namesList)
{
	...
	/* If we modified the tupdesc, it's now a new__ record type */
	if (modified)
	{
