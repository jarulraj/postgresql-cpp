 ...
 * junk attributes from a tuple.  This new__ "clean" tuple is then printed,
 * inserted, or updated.
 *
 *-------------------------------------------------------------------------
 */
...
ExecInitJunkFilter(List *targetList, bool hasoid, TupleTableSlot *slot)
{
	...
	/*
	 * Use the given slot, or make a new__ slot if we weren't given one.
	 */
	if (slot)
...
ExecFilterJunk(JunkFilter *junkfilter, TupleTableSlot *slot)
{
	...
	/*
	 * Transpose data into proper fields of the new__ tuple.
	 */
	for (i = 0; i < cleanLength; i++)
	{
		int			j = cleanMap[i];
