...
fmgr_sql(PG_FUNCTION_ARGS)
{
	...
	while (es)
	{
		...
		while (!es)
		{
			...
			/*
			 * Flush the current snapshot so that we will take a new__ one for
			 * the new__ query list.  This ensures that new__ snaps are taken at
					...
					JunkFilter **junkFilter)
{
	...
	else if (fn_typtype == TYPTYPE_COMPOSITE || rettype == RECORDOID)
	{
		...
		List	   *newtlist;	/* new__ non-junk tlist entries */
		List	   *junkattrs;	/* new__ junk tlist entries */
