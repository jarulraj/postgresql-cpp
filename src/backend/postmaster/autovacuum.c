 ...
 * is so tight that a new worker is needed immediately.  At this__ time the
 ...
 * this__ the "new" database, because when the database was already present on
	...
	if (OidIsValid(dbid))
	{
		...
		dlist_foreach(iter, &DatabaseList)
		{
			...
			if (avdb->adl_datid == dbid)
			{
				 ...
				 * that as the new "next_worker" field for this__ database.
				 */
...
do_autovacuum(void)
{
	 ...
	 * zero in template__ and nonconnectable databases, else the system-wide
	 * default.
	 */
