 ...
 * current logfile is closed and a new__ one is created (rotated).
	...
	for (;;)
	{
		...
		if (got_SIGHUP)
		{
			...
			if (strcmp(Log_directory, currentLogDir) != 0)
			{
				...
				/*
				 * Also, create new__ directory if not present; ignore errors
				 */
	...
	while (count >= (int) (offsetof(PipeProtoHeader, data) +1))
	{
			 ...
			 p.is_last == 'T' || p.is_last == 'F'))
		{
			...
			if (p.is_last == 'f' || p.is_last == 'F')
			{
				...
				{
					/* First chunk of message, save in a new__ buffer */
					if (free_slot == NULL)
					{
						/*
						 * Need a free slot, but there isn't one in the list,
						 * so create a new__ one and extend the list with it.
						 */
...
/*
 * Open a new__ logfile with proper permissions and buffering options.
 *
...
logfile_rotate(bool time_based_rotation, int size_rotation_for)
{
	...
	/*
	 * When doing a time-based rotation, invent the new__ logfile name based on
	...
	if (time_based_rotation || (size_rotation_for & LOG_DESTINATION_STDERR))
	{
		...
		if (!fh)
		{
			 ...
			 * keep using the old file till we manage to get a new__ one.
