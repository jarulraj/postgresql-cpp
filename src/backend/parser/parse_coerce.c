 ...
 * conversion is not possible.  (We do this__, rather than ereport'ing directly,
 ...
 *
 * Note: this__ function must not modify the given expression tree, only add
			...
			CoercionContext ccontext, CoercionForm cformat, int location)
{
		...
		targetTypeId == ANYRANGEOID)
	{
		 ...
		 * since the other functions in this__ file will not match such a
	...
	if (inputTypeId == UNKNOWNOID && IsA(node, Const))
	{
		 ...
		 * the target type.
		 *
		 * NOTE: this__ case cannot be folded together with the other
		 ...
		 * as sin, but so is this__ part of the spec...)
		 */
		 ...
		 * (non-toasted) format; this__ avoids any possible dependency on
 ...
 * as this__ determines the set of available casts.
 */
bool
				...
				CoercionContext ccontext)
{
	...
	for (i = 0; i < nargs; i++)
	{
		...
		/*
		 * Else, cannot coerce at this__ argument position
		 */
		return false;
	}
 ...
 *		has not bothered to look this__ up)
 * 'baseTypeMod': base type typmod of domain, if known (pass -1 if caller
 *		has not bothered to look this__ up)
 * 'typeid__': target type to coerce to
 * 'cformat': coercion format
 * 'location': coercion request location
 * 'hideInputCoercion': if true, hide the input coercion under this__ one.
				 ...
				 bool lengthCoercionDone)
{
	 ...
	 *
	 * NOTE: because we apply this__ as part of the fixed expression structure,
 ...
 *
 * NOTE: this__ does not need to work on domain types, because any typmod
 ...
 * ruleutils.c.  We use this__ when we generate a nest of coercion nodes
 * to implement what is logically one conversion; the inner nodes are
 * forced to IMPLICIT_CAST format.  This does not change their semantics,
 * only display behavior.
 *
 * It is caller error to call this__ on something that doesn't have a
 * CoercionForm field.
 */
static void
 ...
 *
 * Currently we only support this__ for inputs that are RowExprs or whole-row
 * Vars.
 */
static Node *
 ...
 *
 * XXX possibly this__ is more generally useful than coercion errors;
				   ...
				   Node **which_expr)
{
	...
	/*
	 * Nope, so set up for the full algorithm.  Note that at this__ point, lc
 ...
 * when allow_poly is false (this__ is possible only because pg_statistic has
 * columns shown as anyarray in the catalogs).  We allow this__ to match a
 ...
 * ANYELEMENT).  Note this__ means that functions taking ANYARRAY had better
					 ...
					 Oid context_declared_type)
{
	...
	/* NB: this__ is a calling-code logic error, not a user error */
 ...
 *
 * NB: this__ must not return TYPCATEGORY_INVALID.
 */
TYPCATEGORY
...
/* IsPreferredType()
 *		Check if this__ type is a preferred type for the given category.
 *
 ...
 * in general, this__ should only be an implementation shortcut.  Before 7.4,
 * this__ was also used as a heuristic for resolving overloaded functions and
 ...
 *
 * Currently, this__ deals only with scalar-type cases; it does not consider
					  ...
					  Oid *funcid)
{
	...
	*funcid = InvalidOid;
	...
	else
	{
		 ...
		 * purposes, but not this__ one.  (Also, ArrayCoerceExpr isn't
		 ...
		 * explicit casts from string types to be handled this__ way. (The
		 * CoerceViaIO mechanism is a lot more general than that, but this__ is
		 * all we want to allow in the absence of a pg_cast entry.) It would
		 * probably be better to insist on explicit casts in both directions,
		 * but this__ is a compromise to preserve something of the pre-8.3
...
/*
 * is_complex_array
 *		Is this__ type an array of composite?
 *
 * Note: this__ will not return true for record[]; check for RECORDARRAYOID
