...
struct fp_info
{
	...
	Oid			namespace__;		/* other stuff from pg_proc */
	Oid			rettype;
...
fetch_fp_info(Oid func_id, struct fp_info * fip)
{
	...
	fip->namespace__ = pp->pronamespace;
...
HandleFunctionRequest(StringInfo msgBuf)
{
	 ...
	 */
	aclresult = pg_namespace_aclcheck(fip->namespace__, GetUserId(), ACL_USAGE);
	if (aclresult != ACLCHECK_OK)
		aclcheck_error(aclresult, ACL_KIND_NAMESPACE,
					   get_namespace_name(fip->namespace__));
	InvokeNamespaceSearchHook(fip->namespace__, true);
