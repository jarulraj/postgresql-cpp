...
struct dsm_segment
{
	...
	void	   *impl_private;	/* Implementation-specific private__ data. */
 ...
 * each new__ mapping would require an update to the control segment,
...
dsm_postmaster_startup(PGShmemHeader *shim)
{
	...
	/* Determine size for new__ control segment. */
	...
	/*
	 * Loop until we find an unused identifier for the new__ control segment. We
...
/*
 * Create a new__ dynamic shared memory segment.
 */
dsm_segment *
dsm_create(Size size, int flags)
{
	...
	/* Create a new__ segment descriptor. */
	...
	/* Lock the control segment so we can register the new__ segment. */
	...
	/* Enter the handle into a new__ array slot. */
...
dsm_detach(dsm_segment *seg)
{
	...
	if (seg->control_slot != INVALID_CONTROL_SLOT)
	{
		...
		/* If new__ reference count is 1, try to destroy the segment. */
		if (refcnt == 1)
		{
			/*
			 ...
			 * postmaster shutdown time, or when a new__ postmaster is started
	...
	/* Clean up our remaining backend-private__ data structures. */
	if (seg->resowner != NULL)
		ResourceOwnerForgetDSM(seg->resowner, seg);
	dlist_delete(&seg->node);
	pfree(seg);
 ...
 * one of them should first call dsm_create() to establish a new__ shared
