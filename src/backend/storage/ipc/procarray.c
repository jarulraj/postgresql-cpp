...
ProcArrayApplyRecoveryInfo(RunningTransactions running)
{
	 ...
	 * sort them first.
	 *
	 * Some of the new__ xids are top-level xids and some are subtransactions.
...
GetSnapshotData(Snapshot snapshot)
{
	...
	/*
	 * This is a new__ snapshot, so set both refcounts are zero, and mark it as
 ...
 * that atomically with installing the new__ xmin.
 *
...
ProcArrayInstallImportedXmin(TransactionId xmin, TransactionId sourcexid)
{
	...
	for (index = 0; index < arrayP->numProcs; index++)
	{
		...
		/*
		 * We're good.  Install the new__ xmin.  As in GetSnapshotData, set
 ...
 *
 * This is useful to initialize the cutoff xid after which a new__ changeset
...
GetOldestSafeDecodingTransactionId(void)
{
	 ...
	 * running. If no transaction with xid were running concurrently a new__ xid
 ...
 * list expands as new__ XIDs are observed or inferred, and contracts when
 ...
 *	* startup process adding new__ known-assigned XIDs
 ...
 *
 * It's cheap to maintain the sortedness during insertions, since new__ known
 ...
 *
 *	* Adding a new__ XID is O(1) and needs little locking (unless compression
 *		must happen)
