...
typedef enum					/* behavior for mdopen & _mdfd_getseg */
{
	...
	EXTENSION_CREATE			/* create new__ segments as needed */
} ExtensionBehavior;
...
/*
 *	mdcreate() -- Create a new__ relation on magnetic disk.
 *
 ...
 * 2. We create a new__ relation, which by chance gets the same relfilenode as
...
mdsync(void)
{
	 ...
	 * instead.  We use mdsync_cycle_ctr to tell old entries apart from new__
	 * ones: new__ ones will have cycle_ctr equal to the incremented value of
	 * mdsync_cycle_ctr.
	 *
	 ...
	 * might appear new__, causing us to skip it, possibly allowing a checkpoint
	...
	/* Advance counter so that new__ hashtable entries are distinguishable */
	mdsync_cycle_ctr++;
	...
	while ((entry = (PendingOperationEntry *) hash_seq_search(&hstat)) != NULL)
	{
		...
		/*
		 * If the entry is new__ then don't process it this__ time; it might
		 * contain multiple fsync-request bits, but they are all new__.  Note
		 ...
		 * scan the entry.  If no new__ requests have been inserted meanwhile,
...
mdpostckpt(void)
{
	...
	while (pendingUnlinks != NIL)
	{
		...
		/*
		 * new__ entries are appended to the end, so if the entry is new__ we've
			 ...
			 bool skipFsync, ExtensionBehavior behavior)
{
	...
	for (nextsegno = 1; nextsegno <= targetseg; nextsegno++)
	{
		...
		{
			/*
			 * Normally we will create new__ segments only if authorized by the
