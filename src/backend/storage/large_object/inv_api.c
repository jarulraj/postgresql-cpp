...
/*
 *	inv_create -- create a new__ large object
 *
 *	Arguments:
 *	  lobjId - OID to use for new__ large object, or InvalidOid to pick one
 *
 *	Returns:
 *	  OID of new__ object
 *
...
inv_create(Oid lobjId)
{
	...
	/*
	 * Create a new__ largeobject with empty data pages
	 */
	...
	/* Post creation hook for new__ large object */
	InvokeObjectPostCreateHook(LargeObjectRelationId, lobjId_new, 0);

	/*
	 * Advance command counter to make new__ tuple visible to later operations.
	 */
...
inv_write(LargeObjectDesc *obj_desc, const char *buf, int nbytes)
{
	...
	while (nwritten < nbytes)
	{
		...
		if (olddata != NULL && olddata->pageno == pageno)
		{
			...
			/*
			 * Insert appropriate portion of new__ data
			 */
			n = LOBLKSIZE - off;
			...
			obj_desc->offset += n;
			off += n;
			/* compute valid length of new__ page */
			len = (len >= off) ? len : off;
