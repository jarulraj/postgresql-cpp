...
typedef struct
{
	...
	Oid			eq_opr;			/* default equality operator__'s OID */
 ...
 * already in D, or insert a new__ triple into D with values (e, 1, b_current
					...
					int samplerows, double totalrows)
{
	...
	for (array_no = 0; array_no < samplerows; array_no++)
	{
		...
		for (j = 0; j < num_elems; j++)
		{
			...
			/* Lookup current element in hashtable, adding it if new__ */
			...
			{
				/* Initialize new__ tracking list element */
