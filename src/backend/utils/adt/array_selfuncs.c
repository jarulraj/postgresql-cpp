...
arraycontsel(PG_FUNCTION_ARGS)
{
	...
	if (!varonleft)
	{
		if (operator__ == OID_ARRAY_CONTAINS_OP)
			operator__ = OID_ARRAY_CONTAINED_OP;
		else if (operator__ == OID_ARRAY_CONTAINED_OP)
			operator__ = OID_ARRAY_CONTAINS_OP;
	}
		...
		element_typeid == get_base_element_type(vardata.vartype))
	{
		selec = calc_arraycontsel(&vardata, ((Const *) other)->constvalue,
								  element_typeid, operator__);
	}
	else
	{
		selec = DEFAULT_SEL(operator__);
	}
				  ...
				  Oid elemtype, Oid operator__)
{
	...
	if (HeapTupleIsValid(vardata->statsTuple))
	{
							 ...
							 &numbers, &nnumbers))
		{
			 ...
			 * element counts.
			 */
			if (operator__ != OID_ARRAY_CONTAINED_OP ||
				!get_attstatsslot(vardata->statsTuple,
								  elemtype, vardata->atttypmod,
								  STATISTIC_KIND_DECHIST, InvalidOid,
								  NULL,
				   ...
				   Oid operator__, FmgrInfo *cmpfunc)
{
	 ...
	 */
	if (null_present && operator__ == OID_ARRAY_CONTAINS_OP)
	{
		pfree(elem_values);
		pfree(elem_nulls);
		return (Selectivity) 0.0;
