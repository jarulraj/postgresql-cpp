...
numeric		(PG_FUNCTION_ARGS)
{
	...
	if (typmod < (int32) (VARHDRSZ))
	{
		new__ = (Numeric) palloc(VARSIZE(num));
		memcpy(new__, num, VARSIZE(num));
		PG_RETURN_NUMERIC(new__);
	}
			...
			|| !NUMERIC_IS_SHORT(num)))
	{
		...
		if (NUMERIC_IS_SHORT(num))
			new__->choice.n_short.n_header =
				(num->choice.n_short.n_header & ~NUMERIC_SHORT_DSCALE_MASK)
				| (scale << NUMERIC_SHORT_DSCALE_SHIFT);
		else
			new__->choice.n_long.n_sign_dscale = NUMERIC_SIGN(new__) |
...
numeric_mul(PG_FUNCTION_ARGS)
{
	 ...
	 * case of numeric_mul(), which is invoked for the * operator__ on numerics,
