...
/*
 * domain_state_setup - initialize the cache for a new__ domain type.
 *
 * Note: we can't re-use the same cache struct for a new__ domain type,
 * since there's no provision for releasing the DomainConstraintRef.
 * If a call site needs to deal with a new__ domain type, we just leak
