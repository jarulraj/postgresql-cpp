...
array_in(PG_FUNCTION_ARGS)
{
	...
	else
	{
		...
		/* If array dimensions are given, expect '=' operator__ */
...
array_out(PG_FUNCTION_ARGS)
{
	 ...
	 *
	 * +2 allows for assignment operator__ + trailing null
	 */
	bool	   *needquotes,
				needdims = false;
	int			nitems,
 ...
 *		   lowerIndx), creates a new__ array structure for the referred elements
 ...
 *
 * Outputs:
 *	The return value is the new__ array Datum (it's never NULL)
 *
				...
				char elmalign)
{
	...
	/*
	 * Lower bounds of the new__ array are set to 1.  Formerly (before 7.3) we
 ...
 *		  a subscript array) to a new__ value specified by "dataValue".
 *
 * This handles both ordinary varlena arrays and fixed-length arrays.
 *
 * Inputs:
 ...
 *
 * Result:
 *		  A new__ array is returned, just like the old except for the one
 ...
 * positions between the existing elements and the new__ one are set to NULLs.
				  ...
				  char elmalign)
{
	...
	/*
	 * OK, create the new__ array and fill in header/dimensions
	 */
						   ...
						   int elmlen, bool elmbyval, char elmalign)
{
	...
	{
		/*
		 * Allocate adequate space for new__ dimension info.  This is harmless
		 * if we fail later.
		 */
	...
	/* And finally we can insert the new__ element. */
 ...
 *		  by upper and lower subscript values) to new__ values passed as
 *		  another array.
 *
 * This handles both ordinary varlena arrays and fixed-length arrays.
 *
 ...
 *
 * Result:
 *		  A new__ array is returned, just like the old except for the
 ...
 * positions between the existing elements and the new__ ones are set to NULLs.
 * (XXX TODO: allow a corresponding behavior for multidimensional arrays)
 *
				...
				char elmalign)
{
	...
	/*
	 * Compute space occupied by new__ entries, space occupied by replaced
	 * entries, and required space for new__ array.
	 */
	if (newhasnulls)
 ...
 * array_map()
 *
 * Map an array through an arbitrary function.  Return a new__ array with
...
array_map(FunctionCallInfo fcinfo, Oid retType, ArrayMapState *amstate)
{
	...
	/* Allocate temporary arrays for new__ values */
...
array_eq(PG_FUNCTION_ARGS)
{
	...
	else
	{
		...
		/*
		 * apply the operator__ to each pair of array elements.
		 */
		...
		for (i = 0; i < nitems; i++)
		{
			...
			/*
			 * Apply the operator__ to the element pair
			 */
...
hash_array(PG_FUNCTION_ARGS)
{
	...
	for (i = 0; i < nitems; i++)
	{
		 ...
		 * current value by 31 and adding on the new__ element's hash value.
		 *
					  ...
					  bool matchall, void **fn_extra)
{
	...
	/*
	 * Apply the comparison operator__ to each pair of array elements.
	 */
	InitFunctionCallInfoData(locfcinfo, &typentry->eq_opr_finfo, 2,
							 collation, NULL, NULL);
 ...
 *
 * ndim/dim[]/lb[] are dimensions of the original array.  A new__ array with
 ...
 *	dvalue/disnull represent the new__ Datum to append to the array
 ...
 *
 *	astate is working state (can be NULL on first call)
 *	dvalue/disnull represent the new__ sub-array to append to the array
 *	array_type is the array type (must be a valid varlena array type)
 *	rcontext is where to keep working state
 */
ArrayBuildStateArr *
 ...
 *
 *	astate is working state (can be NULL on first call)
 *	dvalue/disnull represent the new__ input to append to the array
 *	input_type is the input datatype (either element or array type)
 *	rcontext is where to keep working state
 */
ArrayBuildStateAny *
					   ...
					   FunctionCallInfo fcinfo)
{
	...
	/* Prepare to apply the comparison operator__ */
	InitFunctionCallInfoData(locfcinfo, &typentry->eq_opr_finfo, 2,
							 collation, NULL, NULL);
							...
							TypeCacheEntry *typentry)
{
	...
	while (left < right)
	{
		...
		else
		{
			...
			/*
			 * Move the thresholds pointer to match new__ "left" index, so we
