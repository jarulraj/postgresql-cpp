...
static Oid	convert_type_name(text *typename__);
		  ...
		  int modechg, Oid ownerId, DropBehavior behavior)
{
	 ...
	 * handle this when the grantee is not public__.
	 */
 ...
 *		name username, text typename__, and text priv name.
 */
Datum
has_type_privilege_name_name(PG_FUNCTION_ARGS)
{
	Name		username = PG_GETARG_NAME(0);
	text	   *typename__ = PG_GETARG_TEXT_P(1);
	text	   *priv_type_text = PG_GETARG_TEXT_P(2);
	Oid			roleid;
	Oid			typeoid;
	AclMode		mode;
	...
	roleid = get_role_oid_or_public(NameStr(*username));
	typeoid = convert_type_name(typename__);
 ...
 *		Check user privileges on a type given
 *		text typename__ and text priv name.
 *		current_user is assumed
 */
Datum
has_type_privilege_name(PG_FUNCTION_ARGS)
{
	text	   *typename__ = PG_GETARG_TEXT_P(0);
	text	   *priv_type_text = PG_GETARG_TEXT_P(1);
	Oid			roleid;
	Oid			typeoid;
	AclMode		mode;
 ...
 *		Check user privileges on a type given
 *		roleid, text typename__, and text priv name.
 */
Datum
 ...
 */
static Oid
convert_type_name(text *typename__)
{
	char	   *typname = text_to_cstring(typename__);
	Oid			oid;
