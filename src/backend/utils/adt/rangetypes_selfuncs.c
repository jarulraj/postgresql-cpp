...
static double calc_rangesel(TypeCacheEntry *typcache, VariableStatData *vardata,
			  RangeType *constval, Oid operator__);
static double default_range_selectivity(Oid operator__);
 ...
 */
static double
default_range_selectivity(Oid operator__)
{
	switch (operator__)
	{
...
rangesel(PG_FUNCTION_ARGS)
{
	...
	if (!get_restriction_variable(root, args, varRelid,
		...
		PG_RETURN_FLOAT8(default_range_selectivity(operator__));
	 ...
	 * in what follows.
	 */
	if (operator__ == OID_RANGE_CONTAINS_ELEM_OP)
	{
	...
	else if (operator__ == OID_RANGE_ELEM_CONTAINED_OP)
	{
		/*
	 ...
	 */
	if (constrange)
		selec = calc_rangesel(typcache, &vardata, constrange, operator__);
	else
		selec = default_range_selectivity(operator__);
			  ...
			  RangeType *constval, Oid operator__)
{
	...
	else
	{
		...
		hist_selec = calc_hist_selectivity(typcache, vardata, constval,
										   operator__);
		if (hist_selec < 0.0)
			hist_selec = default_range_selectivity(operator__);
		 ...
		 */
		if (operator__ == OID_RANGE_CONTAINED_OP)
		{
					  ...
					  RangeType *constval, Oid operator__)
{
	...
	if (operator__ == OID_RANGE_CONTAINS_OP ||
		operator__ == OID_RANGE_CONTAINED_OP)
	{
		if (!(HeapTupleIsValid(vardata->statsTuple) &&
			  get_attstatsslot(vardata->statsTuple,
							   vardata->atttype, vardata->atttypmod,
