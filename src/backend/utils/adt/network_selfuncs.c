...
static Selectivity networkjoinsel_inner(Oid operator__,
					 VariableStatData *vardata1, VariableStatData *vardata2);
static Selectivity networkjoinsel_semi(Oid operator__,
					VariableStatData *vardata1, VariableStatData *vardata2);
...
static Selectivity inet_mcv_join_sel(Datum *mcv1_values,
				  ...
				  float4 *mcv2_numbers, int mcv2_nvalues, Oid operator__);
...
static int	inet_opr_codenum(Oid operator__);
...
networksel(PG_FUNCTION_ARGS)
{
	 ...
	 * by MCV entries.
	 */
	fmgr_info(get_opcode(operator__), &proc);
	...
	if (get_attstatsslot(vardata.statsTuple,
						 ...
						 NULL, NULL))
	{
		int			opr_codenum = inet_opr_codenum(operator__);
		...
		non_mcv_selec = DEFAULT_SEL(operator__);
...
networkjoinsel(PG_FUNCTION_ARGS)
{
	...
	switch (sjinfo->jointype)
	{
		...
		case JOIN_FULL:
			 ...
			 */
			selec = networkjoinsel_inner(operator__, &vardata1, &vardata2);
			break;
		case JOIN_SEMI:
		case JOIN_ANTI:
			/* Here, it's important that we pass the outer var on the left. */
			if (!join_is_reversed)
				selec = networkjoinsel_semi(operator__, &vardata1, &vardata2);
			else
				selec = networkjoinsel_semi(get_commutator(operator__),
											&vardata2, &vardata1);
			break;
		default:
 ...
 */
static Selectivity
networkjoinsel_inner(Oid operator__,
					 VariableStatData *vardata1, VariableStatData *vardata2)
{
	Form_pg_statistic stats;
	...
	opr_codenum = inet_opr_codenum(operator__);
	...
	if ((!mcv1_exists && !hist1_exists) || (!mcv2_exists && !hist2_exists))
		selec = (1.0 - nullfrac1) * (1.0 - nullfrac2) * DEFAULT_SEL(operator__);
 ...
 */
static Selectivity
networkjoinsel_semi(Oid operator__,
					VariableStatData *vardata1, VariableStatData *vardata2)
{
	Form_pg_statistic stats;
	Selectivity selec = 0.0,
