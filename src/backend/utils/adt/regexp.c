 ...
 * make room.  (This might seem to be weighting the new__ entry too heavily,
 * but if we insert new__ entries further back, we'll be unable to adjust to
...
RE_compile_and_cache(text *text_re, int cflags, Oid collation)
{
	...
	/*
	 * Couldn't find it, so try to compile the new__ RE.  To avoid leaking
	...
	/*
	 * Okay, we have a valid new__ item in re_temp; insert it into the storage
