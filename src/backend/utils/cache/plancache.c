 ...
 * parse analysis and rewrite is repeated to build a new__ valid query tree,
				 ...
				 const char *commandTag)
{
	...
	/*
	 * Create and fill the CachedPlanSource struct within the new__ context.
...
RevalidateCachedQuery(CachedPlanSource *plansource)
{
	...
	/*
	 * Allocate new__ query_context and copy the completed querytree into it.
...
/*
 * BuildCachedPlan: construct a new__ CachedPlan from a CachedPlanSource.
 *
				...
				ParamListInfo boundParams)
{
	...
	if (!plansource->is_oneshot)
	{
		...
		/*
		 * Copy plan into the new__ context.
		 */
	...
	/*
	 * Create and fill the CachedPlan struct within the new__ context.
	 */
	...
	/* assign generation number to new__ plan */
			  ...
			  bool useResOwner)
{
	...
	if (!customplan)
	{
		...
		{
			/* Build a new__ generic plan */
			plan = BuildCachedPlan(plansource, qlist, NULL);
			/* Just make real sure plansource->gplan is clear */
			ReleaseGenericPlan(plansource);
			/* Link the new__ generic plan into the plansource */
			...
			/* Update generic_cost whenever we make a new__ generic plan */
...
/*
 * CachedPlanSetParentContext: move a CachedPlanSource to a new__ memory context
 *
