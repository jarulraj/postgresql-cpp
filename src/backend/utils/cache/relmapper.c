 ...
 *
 * Relocation of a normal table is committed (ie, the new__ physical file becomes
 ...
 * safe for the new__ file to replace the old, even if the transaction itself
 ...
 *
 * Install a new__ relfilenode mapping for the specified relation.
 *
 ...
 * apply_map_update
 *
 * Insert a new__ mapping into the given map variable, replacing any existing
...
apply_map_update(RelMapFile *map, Oid relationId, Oid fileNode, bool add_okay)
{
	...
	/* Nope, need to add a new__ mapping */
	if (!add_okay)
...
/*
 * Write out a new__ shared or local map file with the given contents.
 *
...
perform_relmap_update(bool shared, const RelMapFile *updates)
{
	...
	/*
	 * Apply the updates to newmap.  No new__ mappings should appear, unless
...
relmap_redo(XLogReaderState *record)
{
	...
	if (info == XLOG_RELMAP_UPDATE)
	{
		...
		/*
		 * Write out the new__ map and send sinval, but of course don't write a
		 * new__ WAL entry.  There's no surrounding transaction to tell to
