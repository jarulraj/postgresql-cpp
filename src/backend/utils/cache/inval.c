 ...
 *	being valid and the new__ version, if any, becomes valid.  Therefore,
 ...
 *	the new__ tuples; again, that mustn't happen until end of command.
 *
 *	Once we have finished the command, we still need to remember inserted
 *	tuples (including new__ versions of updated tuples), so that we can flush
 ...
 * version and newtuple the new__ version.  This allows avoidance of duplicate
 ...
 * else the relcache won't get updated with the new__ filenode data.
 *
 * Note: because these messages are nontransactional, they won't be captured
