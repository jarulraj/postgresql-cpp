...
lookup_ts_dictionary_cache(Oid dictId)
{
	...
	if (entry == NULL || !entry->isvalid)
	{
		...
		Form_pg_ts_template template__;
		...
		if (entry == NULL)
		{
			...
			/* Create private__ memory context the first time through */
		...
		{
			/* Clear the existing entry's private__ context */
		...
		entry->lexizeOid = template__->tmpllexize;

		if (OidIsValid(template__->tmplinit))
		{
			List	   *dictoptions;
			Datum		opt;
			bool		isnull;
			...
			/*
			 * Init method runs in dictionary's private__ memory context, and we
			...
			entry->dictData =
				DatumGetPointer(OidFunctionCall1(template__->tmplinit,
...
lookup_ts_config_cache(Oid cfgId)
{
	...
	if (entry == NULL || !entry->isvalid)
	{
		...
		while ((maptup = systable_getnext_ordered(mapscan, ForwardScanDirection)) != NULL)
		{
			...
			if (toktype > maxtokentype)
			{
				/* starting a new__ token type, but first save the prior data */
				if (ndicts > 0)
				{
