 ...
 *
 * Create a new__ tuplestore; other types of tuple stores (other than
 ...
 *
 * The new__ pointer initially copies the position of read pointer 0.
...
tuplestore_alloc_read_pointer(Tuplestorestate *state, int eflags)
{
	...
	if (state->status != TSS_INMEM || state->memtupcount != 0)
	{
		if ((state->eflags | eflags) != state->eflags)
			elog(ERROR, "too late to require new__ tuplestore eflags");
	}
...
tuplestore_select_read_pointer(Tuplestorestate *state, int ptr)
{
	...
	switch (state->status)
	{
		...
		case TSS_READFILE:
			 ...
			 * logical position of the new__ read pointer.  In eof_reached
