...
/*
 * hash_create -- create a new__ dynamic hash table
 *
 ...
 * it is unable to create a new__ entry.  The HASH_ENTER_NULL operation is
 ...
 *
 * This is equivalent to removing the entry, making a new__ entry, and copying
 ...
 *
 * Returns TRUE if successful, FALSE if the requested new__ hash key is already
 * present.  Throws error if the specified entry pointer isn't actually a
 * table member.
 *
 * NB: currently, there is no special case for old and new__ hash keys being
 ...
 * partitions, if the new__ hash key would belong to a different partition.
 */
bool
					 ...
					 const void *newKeyPtr)
{
	...
	/*
	 * If old and new__ hash values belong to the same bucket, we need not
	...
	if (bucket != newbucket)
	{
		...
		*oldPrevPtr = currBucket->link;
		...
		/* link into new__ hashbucket chain */
		*prevBucketPtr = currBucket;
		currBucket->link = NULL;
	}

	/* copy new__ key into record */
...
/*
 * create a new__ entry if possible
 */
static HASHBUCKET
...
expand_table(HTAB *hashp)
{
	...
	{
		/* Allocate new__ segment if necessary -- could fail if dir full */
	...
	/* OK, we created a new__ bucket */
	hctl->max_bucket++;

	/*
	 * *Before* changing masks, find old bucket corresponding to same hash
	 * values; values in that bucket may need to be relocated to new__ bucket.
	...
	/*
	 * Relocate records to the new__ bucket.  NOTE: because of the way the hash
...
/*
 * allocate some new__ elements and link them into the free list
 */
static bool
element_alloc(HTAB *hashp, int nelem)
{
	...
	/* prepare to link all the new__ entries into the freelist */
	prevElement = NULL;
