	...
	while (!feof(file) && !ferror(file))
	{
		...
		while (strlen(lineptr) > 0)
		{
			...
			if (list_length(current_field) > 0)
			{
				if (current_line == NIL)
				{
					/* make a new__ line List, record its line number */
		...
		/* we are at real or logical EOL, so force a new__ line List */
		current_line = NIL;
		line_number++;
	}
...
load_hba(void)
{
	...
	/* Loaded new__ file successfully, replace the one we use */
