				...
				float4 prorows)
{
	...
	if (HeapTupleIsValid(oldtup))
	{
		...
		if (oldproc->pronargdefaults != 0)
		{
			...
			/* new__ list can have more defaults than old, advance over 'em */
	...
	/*
	 * Create dependencies for the new__ function.  If we are updating an
	...
	/* Post creation hook for new__ function */
	...
	if (OidIsValid(languageValidator))
	{
		...
		/* Advance command counter so new__ tuple can be seen by validator */
		...
		if (check_function_bodies)
		{
			...
			if (set_items)		/* Need a new__ GUC nesting level */
			{
