...
EnumValuesCreate(Oid enumTypeOid, List *vals)
{
	...
	{
		/*
		 * We assign even-numbered OIDs to all the new__ enum labels.  This
...
/*
 * AddEnumLabel
 *		Add a new__ label to the enum set. By default it goes at
			 ...
			 bool skipIfExists)
{
	...
	/* check length of new__ label is ok */
...
restart:
	...
	{
		/*
		 * Put the new__ label at the end of the list. No change to existing
		 * tuples is required.
		 */
		if (nelems > 0)
		{
	...
	/* Get a new__ OID for the new__ label */
	if (IsBinaryUpgrade)
	{
	...
	{
		/*
		 * Normal case: we need to allocate a new__ Oid for the value.
		 *
		 * We want to give the new__ element an even-numbered Oid if it's safe,
		...
		for (;;)
		{
			...
			/* Get a new__ OID (different from all existing pg_enum tuples) */
	...
	/* Create the new__ pg_enum entry */
	memset(nulls, false, sizeof(nulls));
	values[Anum_pg_enum_enumtypid - 1] = ObjectIdGetDatum(enumTypeOid);
