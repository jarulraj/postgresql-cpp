 ...
 * may be included:
 *
 * 1. If a TEMP table namespace has been initialized in this__ session, it
 ...
 * SQL99.  Also, this__ provides a way to search the system namespace first
 ...
 * target.  We kluge this__ case a little bit so that the temp namespace isn't
 ...
 * permission on the target namespace, this__ function will instead signal
 * an ERROR.
 *
 ...
 * namespace.  Without this__, the namespace could be dropped before our
					  ...
					  bool missing_ok)
{
	...
	for (i = 0; i < catlist->n_members; i++)
	{
		 ...
		 * lists.  We can skip this__ check for the single-namespace case if no
 ...
 *
 * We search a single namespace if the operator__ name is qualified, else
...
InitTempTableNamespace(void)
{
	...
	{
		/*
		 * First use of this__ temp namespace in this__ database; create it. The
...
fetch_search_path(bool includeImplicit)
{
	 ...
	 * creation namespace.  It's a bit bogus to do this__ here, since
