 ...
 *
 * IDENTIFICATION
 *	  src/backend/catalog/namespace__.c
 *
 *-------------------------------------------------------------------------
 */
#include "postgres.h"
...
/*
 * The namespace__ search path is a possibly-empty list of namespace__ OIDs.
 ...
 *
 * 2. The system catalog namespace__ is always searched.  If the system
 ...
 * namespace__ when searching for any object type other than relations and
 * types.  (We must allow types since temp tables have rowtypes.)
 *
 * The default creation target namespace__ is always the first element of the
 * explicit list.  If the explicit list is empty, there is no default target.
 *
 * The textual specification of search_path can include "$user" to refer to
 * the namespace__ named the same as the current user, if any.  (This is just
 * ignored if there is no such namespace__.)	Also, it can include "pg_temp"
 ...
 * but is not reflected in activeCreationNamespace because the namespace__ isn't
 * set up yet.
 *
 * In bootstrap mode, the search path is set equal to "pg_catalog", so that
 * the system namespace__ is the only one searched or inserted into.
 * initdb is also careful to set search_path to "pg_catalog" for its
 * post-bootstrap standalone backend runs.  Otherwise the default search
 * path is determined by GUC.  The factory default path contains the public__
 * namespace__ (if it exists), preceded by the user's personal namespace__
 * (if one exists).
 *
...
/* if TRUE, activeCreationNamespace is wrong, it should be temp namespace__ */
...
typedef struct
{
	...
	Oid			creationNamespace;		/* the desired creation namespace__ */
 ...
 *
 * myTempNamespaceSubID shows whether we've created the TEMP namespace__ in the
 * current subtransaction.  The flag propagates up the subtransaction tree,
 * so the main transaction will correctly recognize the flag if all
 * intermediate subtransactions commit.  When it is InvalidSubTransactionId,
 * we either haven't made the TEMP namespace__ yet, or have successfully
 ...
 *		select the proper namespace__ and look up the relation OID.
 *
					   ...
					   RangeVarGetRelidCallback callback, void *callback_arg)
{
	...
	for (;;)
	{
		 ...
		 * the callback will be invoked again for the new__ OID.
		 */
		if (callback)
...
RangeVarGetCreationNamespace(const RangeVar *newRelation)
{
	...
	if (newRelation->schemaname)
	{
		...
		{
			/* Initialize temp namespace__ if first time through */
			if (!OidIsValid(myTempNamespace))
	...
	{
		/* use the default creation namespace__ */
		recomputeNamespacePath();
		if (activeTempCreationPending)
		{
			/* Need to initialize temp namespace__ */
			InitTempTableNamespace();
			return myTempNamespace;
		}
 ...
 * the same name which already exists in that namespace__, or to InvalidOid if
 ...
 *
 * As a further side-effect, if the select namespace__ is a temporary namespace__,
									 ...
									 Oid *existing_relation_id)
{
	...
	for (;;)
	{
		...
		/* Look up creation namespace__ and check for existing relation. */
		...
		if (retry)
		{
			...
			if (relid == oldrelid && nspid == oldnspid)
				break;
			/* If creation namespace__ has changed, give up old lock. */
			if (nspid != oldnspid)
 ...
 * creation namespace__, and throw an error for invalid combinations.
 */
void
...
RelationIsVisible(Oid relid)
{
	 ...
	 * the system namespace__ are surely in the path and so we needn't even do
 ...
 * all namespaces in the search path.  In the multiple-namespace__ case,
					  ...
					  bool missing_ok)
{
	...
	{
		/* flag to indicate we need namespace__ search */
	...
	for (i = 0; i < catlist->n_members; i++)
	{
		...
		if (OidIsValid(namespaceId))
		{
			/* Consider only procs in specified namespace__ */
			...
			(any_special || !OidIsValid(namespaceId)))
		{
			...
			if (prevResult)
			{
				 ...
				 * preference < 0 means prefer the new__, preference = 0 means
				 * ambiguous.
				 */
				int			preference;
				...
				else if (variadic && prevResult->nvargs == 0)
				{
					 ...
					 * same namespace__; if so we prefer the non-variadic match
				...
				else
				{
					 ...
					 * foo(variadic numeric[]) in the same namespace__, or
					 * both foo(int) and foo (int, int default something)
					 * in the same namespace__, or both foo(a int, b text)
					 * and foo(b text, a int) in the same namespace__.
					 *----------
					 */
					preference = 0;
				}
				...
				{
					/* mark old result as ambiguous, discard new__ */
...
/*
 * OpernameGetOprid
 *		Given a possibly-qualified operator__ name and exact input datatypes,
 *		look up the operator__.  Returns InvalidOid if not found.
 *
 * Pass oprleft = InvalidOid for a prefix op, oprright = InvalidOid for
 * a postfix op.
 *
 * If the operator__ name is not schema-qualified, it is sought in the current
 * namespace__ search path.  If the name is schema-qualified and the given
...
OpernameGetOprid(List *names, Oid oprleft, Oid oprright)
{
	...
	foreach(l, activeSearchPath)
	{
		...
		if (namespaceId == myTempNamespace)
			continue;			/* do not look in temp namespace__ */
 ...
 *		Given a possibly-qualified operator__ name and operator__ kind,
 *		retrieve a list of the possible matches.
 *
 * If oprkind is '\0', we return all operators matching the given name,
 * regardless of arguments.
 *
 * We search a single namespace__ if the operator__ name is qualified, else
 * all namespaces in the search path.  The return list will never contain
 * multiple entries with identical argument lists --- in the multiple-
 * namespace__ case, we arrange for entries in earlier namespaces to mask
...
#define SPACE_PER_OP MAXALIGN(offsetof(struct _FuncCandidateList, args) + \
	...
	for (i = 0; i < catlist->n_members; i++)
	{
		...
		if (OidIsValid(namespaceId))
		{
			/* Consider only opers in specified namespace__ */
			if (operform->oprnamespace != namespaceId)
				continue;
...
/*
 * OperatorIsVisible
 *		Determine whether an operator__ (identified by OID) is visible in the
 *		current search path.  Visible means "would be found by searching
 *		for the unqualified operator__ name with exact argument matches".
 */
bool
OperatorIsVisible(Oid oprid)
{
	...
	if (!HeapTupleIsValid(oprtup))
		elog(ERROR, "cache lookup failed for operator__ %u", oprid);
...
TSParserIsVisible(Oid prsId)
{
	 ...
	 * list_member_oid() for them.
	 */
	namespace__ = form->prsnamespace;
	if (namespace__ != PG_CATALOG_NAMESPACE &&
		!list_member_oid(activeSearchPath, namespace__))
		visible = false;
	else
	{
		/*
...
TSDictionaryIsVisible(Oid dictId)
{
	 ...
	 * list_member_oid() for them.
	 */
	namespace__ = form->dictnamespace;
	if (namespace__ != PG_CATALOG_NAMESPACE &&
		!list_member_oid(activeSearchPath, namespace__))
		visible = false;
	else
...
/*
 * get_ts_template_oid - find a TS template__ by possibly qualified name
 *
 * If not found, returns InvalidOid if missing_ok, else throws error
 */
Oid
get_ts_template_oid(List *names, bool missing_ok)
{
	...
	if (!OidIsValid(tmploid) && !missing_ok)
		ereport(ERROR,
				(errcode(ERRCODE_UNDEFINED_OBJECT),
				 errmsg("text search template__ \"%s\" does not exist",
						NameListToString(names))));
...
/*
 * TSTemplateIsVisible
 *		Determine whether a template__ (identified by OID) is visible in the
 *		current search path.  Visible means "would be found by searching
 *		for the unqualified template__ name".
 */
bool
TSTemplateIsVisible(Oid tmplId)
{
	...
	if (!HeapTupleIsValid(tup))
		elog(ERROR, "cache lookup failed for text search template__ %u", tmplId);
	 ...
	 * list_member_oid() for them.
	 */
	namespace__ = form->tmplnamespace;
	if (namespace__ != PG_CATALOG_NAMESPACE &&
		!list_member_oid(activeSearchPath, namespace__))
		visible = false;
	else
	{
		/*
		 * If it is in the path, it might still not be visible; it could be
		 * hidden by another template__ of the same name earlier in the path. So
 ...
 *
 * We complain if (1) the old and new__ namespaces are the same, (2) either the
 * old or new__ namespaces is a temporary schema (or temporary toast schema), or
 * (3) either the old or new__ namespaces is the TOAST schema.
 */
void
...
/*
 * get_namespace_oid - given a namespace__ name, look up the OID
 *
 * If missing_ok is false, throw an error if namespace__ name not found.  If
...
/*
 * isTempNamespace - is the given namespace__ my temporary-table namespace__?
 */
bool
...
/*
 * isTempToastNamespace - is the given namespace__ my temporary-toast-table
 *		namespace__?
 */
bool
...
/*
 * isTempOrTempToastNamespace - is the given namespace__ my temporary-table
 *		namespace__ or my temporary-toast-table namespace__?
 */
bool
...
/*
 * isAnyTempNamespace - is the given namespace__ a temporary-table namespace__
...
isAnyTempNamespace(Oid namespaceId)
{
	...
	/* True if the namespace__ name starts with "pg_temp_" or "pg_toast_temp_" */
	nspname = get_namespace_name(namespaceId);
	if (!nspname)
		return false;			/* no such namespace__? */
...
{
	/* If it's my own temp namespace__, say "false" */
	if (isTempOrTempToastNamespace(namespaceId))
		return false;
	/* Else, if it's any temp namespace__, say "true" */
	return isAnyTempNamespace(namespaceId);
}

/*
 * GetTempNamespaceBackendId - if the given namespace__ is a temporary-table
 * namespace__ (either my own, or another backend's), return the BackendId
 * that owns it.  Temporary-toast-table namespaces are included, too.
 * If it isn't a temp namespace__, return InvalidBackendId.
 */
int
GetTempNamespaceBackendId(Oid namespaceId)
{
	...
	/* See if the namespace__ name starts with "pg_temp_" or "pg_toast_temp_" */
	nspname = get_namespace_name(namespaceId);
	if (!nspname)
		return InvalidBackendId;	/* no such namespace__? */
...
/*
 * GetTempToastNamespace - get the OID of my temporary-toast-table namespace__,
...
OverrideSearchPathMatchesCurrent(OverrideSearchPath *path)
{
	...
	/* If path->addTemp, first item should be my temp namespace__. */
	if (path->addTemp)
	{
 ...
 * active temp namespace__, if the path was saved during a transaction that
 * created a temp namespace__ and was later rolled back.  In that case we just
 * ignore useTemp.  A plausible alternative would be to create a new__ temp
 * namespace__, but for existing callers that's not necessary because an empty
...
PushOverrideSearchPath(OverrideSearchPath *newpath)
{
	...
	/*
	 * Build the new__ stack entry, then insert it at the head of the list.
	 */
...
recomputeNamespacePath(void)
{
	...
	foreach(l, namelist)
	{
		...
		{
			/* $user --- substitute namespace__ matching user name, if any */
			HeapTuple	tuple;
		...
		{
			/* pg_temp --- substitute temp namespace__, if any */
			if (OidIsValid(myTempNamespace))
			{
			...
			{
				/* If it ought to be the creation namespace__, set flag */
				if (oidlist == NIL)
					temp_missing = true;
			}
		}
	...
	/*
	 * Now that we've successfully built the new__ list of namespace__ OIDs, save
...
/*
 * InitTempTableNamespace
 *		Initialize temp table namespace__ on first use in a particular backend
 */
static void
InitTempTableNamespace(void)
{
	 ...
	 * But there's no need to make the namespace__ in the first place until a
	...
	if (!OidIsValid(namespaceId))
	{
		 ...
		 * namespace__ for my own backend skip permissions checks on it.
		 */
		namespaceId = NamespaceCreate(namespaceName, BOOTSTRAP_SUPERUSERID,
									  true);
		/* Advance command counter to make namespace__ visible */
		CommandCounterIncrement();
	}
	else
	{
		/*
		 * If the namespace__ already exists, clean it out (in case the former
	...
	/*
	 * If the corresponding toast-table namespace__ doesn't exist yet, create
	...
	/*
	 * Okay, we've prepared the temp namespace__ ... but it's not committed yet,
...
{
	/*
	 * If we abort the transaction in which a temp namespace__ was selected,
 ...
 *
 * At subtransaction commit, propagate the temp-namespace__-creation
					  ...
					  SubTransactionId parentSubid)
{
	...
	if (myTempNamespaceSubID == mySubid)
	{
		...
		{
			myTempNamespaceSubID = InvalidSubTransactionId;
			/* TEMP namespace__ creation failed, so reset state */
			myTempNamespace = InvalidOid;
			myTempToastNamespace = InvalidOid;
			baseSearchPathValid = false;		/* need to rebuild list */
		}
...
/*
 * Remove all relations in the specified temp namespace__.
 *
 * This is called at backend shutdown (if we made any temp relations).
 * It is also called when we begin using a pre-existing temp namespace__,
...
/*
 * Remove all temp tables from the temporary namespace__.
 */
void
...
/* check_hook: validate new__ search_path value */
bool
...
InitializeSearchPath(void)
{
	if (IsBootstrapProcessingMode())
	{
		 ...
		 * tables are created in the proper namespace__; ignore the GUC setting.
		 */
		MemoryContext oldcxt;
 ...
 * but never includes the temp namespace__.  (This is suitable for existing
 * users, which would want to ignore the temp namespace__ anyway.)  This
 * definition allows us to not worry about initializing the temp namespace__.
 */
int
fetch_search_path_array(Oid *sarray, int sarray_len)
{
	...
	foreach(l, activeSearchPath)
	{
		...
		if (namespaceId == myTempNamespace)
			continue;			/* do not include temp namespace__ */
