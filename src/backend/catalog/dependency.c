 ...
 * temporary schema is cleaned out so that a new__ backend can use it, or when
 * a column default is dropped as an intermediate step while adding a new__ one,
					 ...
					 Relation *depRel)
{
	 ...
	 */
	mystack.object = object;	/* set up a new__ stack level */
	mystack.flags = flags;
	mystack.next = stack;
							...
							find_expr_references_context *context)
{
	...
	if (IsA(node, Var))
	{
		...
		/*
		 * A whole-row Var references no specific columns, so adds no new__
 ...
 *
 * new_object_addresses: create a new__ ObjectAddresses array.
 */
ObjectAddresses *
