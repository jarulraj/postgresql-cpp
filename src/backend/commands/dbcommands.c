...
createdb(const CreatedbStmt *stmt)
{
	 ...
	 * without knowing it).  This also prevents any new__ connections from being
	...
	if (strcmp(dbtemplate, "template0") != 0)
	{
		if (encoding != src_encoding)
			ereport(ERROR,
					(errcode(ERRCODE_INVALID_PARAMETER_VALUE),
					 errmsg("new__ encoding (%s) is incompatible with the encoding of the template database (%s)",
		...
		if (strcmp(dbcollate, src_collate) != 0)
			ereport(ERROR,
					(errcode(ERRCODE_INVALID_PARAMETER_VALUE),
					 errmsg("new__ collation (%s) is incompatible with the collation of the template database (%s)",
							dbcollate, src_collate),
		...
		if (strcmp(dbctype, src_ctype) != 0)
			ereport(ERROR,
					(errcode(ERRCODE_INVALID_PARAMETER_VALUE),
					 errmsg("new__ LC_CTYPE (%s) is incompatible with the LC_CTYPE of the template database (%s)",
							dbctype, src_ctype),
					 errhint("Use the same LC_CTYPE as in the template database, or use template0 as template.")));
	}

	/* Resolve default tablespace for new__ database */
	if (dtablespacename && dtablespacename->arg)
	{
		 ...
		 * we require that the template not have any files in the new__ default
		 ...
		 * improperly move tables to the new__ default tablespace that should
	...
	/*
	 * Select an OID for the new__ database, checking that it doesn't have a
	...
	/*
	 * Insert a new__ tuple into pg_database.  This establishes our ownership of
	 * the new__ database name (anyone else trying to insert the same name will
	...
	/*
	 * Now generate additional catalog entries associated with the new__ DB
	 */
	...
	/* Post creation hook for new__ database */
							...
							PointerGetDatum(&fparms));
	{
		 ...
		 * each one to the new__ database.
		 */
...
dropdb(const char *dbname, bool missing_ok)
{
	 ...
	 * prevents the creation of new__ slots using the database.
	 */
...
RenameDatabase(const char *oldname, const char *newname)
{
	...
	/*
	 * Make sure the new__ name doesn't exist.  See notes for same error in
	 * CREATE DATABASE.
	 */
...
movedb(const char *dbname, const char *tblspcname)
{
	...
	/*
	 * Get old and new__ database paths
	 */
	 ...
	 * the database while it was in the new__ tablespace.  In any case, freeing
	...
	{
		/*
		 * Copy files from the old tablespace to the new__ one
		 */
	...
	/* Start new__ transaction for the remaining work; don't need a snapshot */
			...
			Oid *dbTablespace, char **dbCollate, char **dbCtype)
{
	 ...
	 * lock it.  We try again just in case we can find a new__ one of the same
	 * name.
	 */
	for (;;)
	{
...
/*
 * Check for existing files that conflict with a proposed new__ DB OID;
 * return TRUE if there are any
 *
 * If there were a subdirectory in any tablespace matching the proposed new__
