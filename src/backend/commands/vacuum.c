...
get_rel_oids(Oid relid, const RangeVar *vacrel)
{
	...
	else if (vacrel)
	{
		 ...
		 * going to commit this__ transaction and begin a new one between now
		 * and then.
		 */
					   ...
					   double scanned_tuples)
{
	 ...
	 * statistics in this__ situation, since no new information has been
	 * provided.)
	 */
	if (scanned_pages == 0)
 ...
 *		safe since the new values are correct whether or not this__ transaction
...
vacuum_rel(Oid relid, RangeVar *relation, int options, VacuumParams *params)
{
	...
	/* all done with this__ class, but hold lock until commit */
	if (onerel)
		relation_close(onerel, NoLock);
