...
/*
 * vac_estimate_reltuples() -- estimate the new__ value for pg_class.reltuples
 *
					   ...
					   double scanned_tuples)
{
	 ...
	 * probably did, if the new__ density measurement is different. But over
	 * repeated cycles, the value of reltuples will converge towards the
	 * correct value, if repeated measurements show the same new__ density.
	 *
					...
					bool in_outer_xact)
{
	 ...
	 * indicating it has no new__ data.
	 *
	 * Ordinarily, we don't let relfrozenxid go backwards: if things are
	 * working correctly, the only way the new__ frozenxid could be older would
...
vac_update_datfrozenxid(void)
{
	 ...
	 * used on pg_class for new__ tables.  See AddNewRelationTuple().
	 */
				  ...
				  MultiXactId lastSaneMinMulti)
{
	 ...
	 *
	 * Note: we need not worry about a race condition with new__ entries being
	 * inserted by CREATE DATABASE.  Any such entry will have a copy of some
	 * existing DB's datfrozenxid, and that source DB cannot be ours because
	 * of the interlock against copying a DB containing an active backend.
	 * Hence the new__ entry will not reduce the minimum.  Also, if two VACUUMs
	...
	/*
	 * Update the wrap limit for GetNewTransactionId and creation of new__
