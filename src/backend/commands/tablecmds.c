 ...
 * Phase 3 copy (this__ could be either a new__ column with a non-null default, or
 ...
 * ownerId: if not InvalidOid, use this__ as the new__ relation's owner.
...
ATRewriteTables(AlterTableStmt *parsetree, List **wqueue, LOCKMODE lockmode)
{
	...
	foreach(ltab, *wqueue)
	{
		...
		if (tab->rewrite > 0)
		{
			 ...
			 * to do this__ here so that buffers for the new__ relfilenode will
					  ...
					  AlterTableCmd *cmd, LOCKMODE lockmode)
{
	...
	TypeName   *typename__ = def->typename__;
					  ...
					  AlterTableCmd *cmd, LOCKMODE lockmode)
{
	 ...
	 * can coerce it to the new__ datatype.  (We must do this__ before changing
					 ...
					 List **wqueue, LOCKMODE lockmode, bool rewrite)
{
	 ...
	 * Note this__ could result in creation of entirely new__ work-queue entries.
	 *
